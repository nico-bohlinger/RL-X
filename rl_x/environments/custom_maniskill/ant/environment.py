...
class AntRobot(BaseAgent):
    uid = "ant"
    mjcf_path = (Path(__file__).resolve().parent / "data" / "ant.xml").as_posix()
    fix_root_link = False
...
@register_env("ManiSkill-Ant", max_episode_steps=1000)
class AntEnv(BaseEnv):
    agent: Union[AntRobot]
    SUPPORTED_REWARD_MODES = ("normalized_dense", "dense", "none")


    def __init__(self, *args, robot_uids=AntRobot, move_speed=2.0, **kwargs):
