...
class DefaultActionDelay:
    def __init__(self, env):
        ...
        self.max_nr_delay_steps = env.robot_config["locomotion_envs"]["default"]["domain_randomization"]["action_delay"]["max_nr_delay_steps"]
        self.mixed_chance = env.robot_config["locomotion_envs"]["default"]["domain_randomization"]["action_delay"]["mixed_chance"]
    ...
    def setup(self, internal_state):
        internal_state["action_history"] = jnp.zeros((self.max_nr_delay_steps + 1, self.env.initial_mjx_model.nu))
    ...
    def delay_action(self, action, internal_state, key):
        current_nr_delay_steps = jnp.where(internal_state["action_current_mixed"], jax.random.randint(key, (1,), 0, self.max_nr_delay_steps+1)[0], internal_state["action_current_nr_delay_steps"])
