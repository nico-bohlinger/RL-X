...
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.state import State
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.box_space import BoxSpace
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.viewer import MujocoViewer
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.control_functions.handler import get_control_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.command_functions.handler import get_command_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.domain_randomization.initial_state_functions.handler import get_initial_state_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.sampling_functions.handler import get_sampling_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.reward_functions.handler import get_reward_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.termination_functions.handler import get_termination_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.domain_randomization.action_delay_functions.handler import get_domain_randomization_action_delay_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.domain_randomization.seen_robot_functions.handler import get_domain_randomization_seen_robot_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.domain_randomization.unseen_robot_functions.handler import get_domain_randomization_unseen_robot_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.domain_randomization.observation_noise_functions.handler import get_observation_noise_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.domain_randomization.joint_dropout_functions.handler import get_joint_dropout_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.exteroceptive_observation_functions.handler import get_exteroceptive_observation_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.terrain_functions.handler import get_terrain_function
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.mjwarp_hfield_patch import apply_patch
from rl_x.environments.custom_mujoco.robot_locomotion.mjx_warp.buffer_sizing import measure_per_world_buffer_sizes
...
class LocomotionEnv:
    def __init__(self, robot_config, runner_mode, render, env_config, nr_envs):
        ...
        if "hfield" in env_config["terrain"]["type"]:
            self.hfield_resolution = env_config["terrain"]["hfield_resolution"]
            for i in range(nr_envs):
                xml_handle.asset.insert("hfield", i, name=f"empty_hfield_{i}", nrow=self.hfield_resolution, ncol=self.hfield_resolution, size="4 4 30.0 0.125")
            floor = xml_handle.find("geom", "floor")
            floor.type = "hfield"
            floor.hfield = "empty_hfield_0"
        ...
        if "hfield" in env_config["terrain"]["type"]:
            hfield_geom_names = [mujoco.mj_id2name(self.initial_mj_model, mujoco.mjtObj.mjOBJ_GEOM, geom_id) for geom_id in range(self.initial_mj_model.ngeom)]
            foot_geom_ids = [geom_id for geom_id, geom_name in enumerate(hfield_geom_names) if geom_name and "foot" in geom_name]
            max_foot_rbound = float(np.max(self.initial_mj_model.geom_rbound[foot_geom_ids]))
            hfield_cell_size_in_meters = (2.0 * float(self.initial_mj_model.hfield_size[0][0])) / self.hfield_resolution
            cells_per_foot = int(np.ceil(2.0 * max_foot_rbound / hfield_cell_size_in_meters)) + 2
            hfield_max_contacts_per_pair = max(64, 2 * cells_per_foot * cells_per_foot)
            apply_patch(self.hfield_resolution, self.hfield_resolution, hfield_max_contacts_per_pair)
    ...
    def render(self, state):
        ...
        for field in mjx.Model.fields():
            if field.type in [jax.Array, np.ndarray]:
                ...
                mjx_value = getattr(mjx_model, field_name, None)
                if mjx_value is None:
                    continue
                mj_value = getattr(mj_model, field_name)
                mjx_value = np.array(mjx_value)
                if mjx_value.shape != mj_value.shape and mjx_value.ndim > 0 and mjx_value.shape[0] == self.nr_envs:
        ...
        if self.uses_hfield and state.info_episode_store["episode_step"][env_id] == 1:
            mujoco.mjr_uploadHField(mj_model, self.viewer.context, 0)

        data = self.render_data
        data.qpos[:] = np.array(state.data.qpos[env_id])
        data.qvel[:] = np.array(state.data.qvel[env_id])
        data.ctrl[:] = np.array(state.data.ctrl[env_id])
        ...
        if self.runner_mode == "test":
            ...
            if explicit_velocity_commands:
                max_command_velocity = np.array(state.internal_state["max_command_velocity"])[env_id]
                goal_velocities = jnp.array([goal_x_velocity, goal_y_velocity, goal_yaw_velocity])
                goal_velocities = jnp.where(jnp.abs(goal_velocities) < (self.command_function.zero_clip_threshold_percentage * max_command_velocity), 0.0, goal_velocities)
                goal_velocities = jnp.clip(goal_velocities, -max_command_velocity, max_command_velocity)
        ...
        if self.add_goal_arrow:
            goal_velocities = np.array(state.internal_state["goal_velocities"][env_id])
            trunk_rotation = np.array(state.internal_state["imu_orientation_euler"][env_id])[2]
    ...
    def reset(self, keys, eval_mode):
        ...
        env_curriculum_coeff = jnp.where(eval_mode, jnp.ones(nr_envs), jnp.zeros(nr_envs))
        internal_state = {
            "in_eval_mode": jnp.full(nr_envs, eval_mode, dtype=bool),
    ...
    def merge_done(self, done, done_state, not_done_state):
        ...
        # hfield_data is the flat (nr_envs * nr_cells,) per-world heightfield buffer, so it has no
        # leading env axis and is skipped by the tree_map above; merge the done envs' terrain explicitly.
        if "hfield" in self.env_config["terrain"]["type"]:
            hfield_done = done_state.mjx_model.hfield_data.reshape(self.nr_envs, -1)
            hfield_keep = not_done_state.mjx_model.hfield_data.reshape(self.nr_envs, -1)
            hfield_merged = jnp.where(jnp.reshape(done, (self.nr_envs, 1)), hfield_done, hfield_keep).reshape(-1)
            mjx_model = mjx_model.replace(hfield_data=hfield_merged)
        # The Warp-backed contact / constraint buffers do not carry a leading env axis, so only the
        # per-env qpos/qvel/ctrl/time are merged; all derived quantities are recomputed by the next mjx.step
