...
class RandomCommands:
    ...
    def init(self, internal_state):
        internal_state["actuator_joint_keep_nominal"] = jnp.tile(self.default_actuator_joint_keep_nominal[None], (self.env.nr_envs, 1))
    ...
    def get_next_command(self, internal_state, should_sample_commands, subkey):
        ...
        max_command_velocity = internal_state["max_command_velocity"][:, None]
        goal_velocities = jax.random.uniform(velocity_sampling_key, (nr_envs, 3), minval=-max_command_velocity, maxval=max_command_velocity)
        ...
        goal_velocities = jnp.where(jax.random.bernoulli(all_zeroing_key, self.all_zero_chance, (nr_envs, 1)), jnp.zeros((nr_envs, 3)), goal_velocities)
        goal_velocities = jnp.where(jax.random.uniform(single_zeroing_key, (nr_envs, 3)) < self.single_zero_chance, 0.0, goal_velocities)

        internal_state["goal_velocities"] = jnp.where(should_sample_commands[:, None], goal_velocities, internal_state["goal_velocities"])

        actuator_joint_keep_nominal = jnp.where(jnp.all(goal_velocities == 0.0, axis=-1, keepdims=True), jnp.ones((nr_envs, self.env.nr_actuator_joints), dtype=bool), self.default_actuator_joint_keep_nominal[None])

        internal_state["actuator_joint_keep_nominal"] = jnp.where(should_sample_commands[:, None], actuator_joint_keep_nominal, internal_state["actuator_joint_keep_nominal"])
