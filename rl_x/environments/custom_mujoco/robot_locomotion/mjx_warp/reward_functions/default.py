...
class DefaultReward:
    ...
    def reward_and_info(self, data, mjx_model, internal_state, action, info):
        ...
        # Joint torque reward
        torque_norm = jnp.mean(jnp.square(data.qfrc_actuator[:, self.env.actuator_joint_mask_qvel]), axis=-1)
        torque_reward = curriculum_coeff * self.joint_torque_coeff * -torque_norm

        # Power draw penalty reward
        power_draw = jnp.mean(jnp.maximum(data.qfrc_actuator[:, self.env.actuator_joint_mask_qvel] * data.qvel[:, self.env.actuator_joint_mask_qvel], 0.0), axis=-1)
