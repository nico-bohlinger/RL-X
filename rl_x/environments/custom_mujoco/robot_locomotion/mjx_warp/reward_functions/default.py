...
class DefaultReward:
    def __init__(self, env):
        ...
        self.feet_orientation_coeff = env.env_config["reward"]["feet_orientation_coeff"] * env.dt * env.foot_type_int

        self.feet_symmetry_pairs = env.feet_symmetry_pairs


    def init(self, internal_state, mjx_model):
        internal_state["joint_position_limits"] = jnp.tile(self.calculate_joint_position_limits(mjx_model)[None], (self.env.nr_envs, 1, 1))
    ...
    def handle_model_change(self, internal_state, mjx_model, should_change):
        joint_position_limits = self.calculate_joint_position_limits(mjx_model)
        if joint_position_limits.ndim == 2:
            joint_position_limits = jnp.broadcast_to(joint_position_limits, internal_state["joint_position_limits"].shape)
        internal_state["joint_position_limits"] = jnp.where(should_change[:, None, None], joint_position_limits, internal_state["joint_position_limits"])


    def setup(self, internal_state):
        internal_state["feet_time_on_ground"] = jnp.zeros((self.env.nr_envs, self.env.nr_feet))
        internal_state["feet_time_in_air"] = jnp.zeros((self.env.nr_envs, self.env.nr_feet))
        internal_state["previous_actuator_joint_velocities"] = jnp.zeros((self.env.nr_envs, self.env.nr_actuator_joints))
        internal_state["previous_imu_linear_velocity"] = jnp.zeros((self.env.nr_envs, self.env.imu_linear_velocity_sensor_dim))
        internal_state["sum_tracking_performance_percentage"] = jnp.zeros(self.env.nr_envs)
    ...
    def reward_and_info(self, data, mjx_model, internal_state, action, info):
        ...
        xy_velocity_difference_norm = jnp.sum(jnp.square(xy_difference), axis=-1)
        tracking_temperature_scale = jnp.maximum(jnp.square(internal_state["max_command_velocity"]), 1e-6)
        tracking_xy_velocity_command_reward = self.tracking_xy_velocity_command_coeff * jnp.exp(-xy_velocity_difference_norm / (self.tracking_xy_temperature * tracking_temperature_scale))
        ...
        tracking_yaw_velocity_command_reward = self.tracking_yaw_velocity_command_coeff * jnp.exp(-yaw_velocity_difference_norm / (self.tracking_yaw_temperature * tracking_temperature_scale))
        ...
        imu_acceleration_norm = jnp.mean(jnp.square((current_imu_linear_velocity - internal_state["previous_imu_linear_velocity"]) / self.env.dt), axis=-1)
        imu_acceleration_reward = curriculum_coeff * self.imu_acceleration_coeff * -imu_acceleration_norm

        # Angular velocity reward
        angular_velocity_norm = jnp.sum(jnp.square(current_imu_angular_velocity[:, :2]), axis=-1)
        angular_velocity_reward = curriculum_coeff * self.roll_pitch_vel_coeff * -angular_velocity_norm

        # Angular position reward
        roll_pitch_position_norm = jnp.sum(jnp.square(internal_state["imu_orientation_euler"][:, :2]), axis=-1)
        ...
        actuator_joint_nominal_diff_norm = jnp.mean(jnp.square((data.qpos[:, self.env.actuator_joint_mask_qpos] * internal_state["actuator_joint_keep_nominal"]) - (internal_state["actuator_joint_nominal_positions"] * internal_state["actuator_joint_keep_nominal"])), axis=-1)
        ...
        lower_limit_penalty = -jnp.minimum(joint_positions - internal_state["joint_position_limits"][:, self.env.actuator_joint_mask_joints - 1, 0], 0.0).mean(axis=-1)
        upper_limit_penalty = jnp.maximum(joint_positions - internal_state["joint_position_limits"][:, self.env.actuator_joint_mask_joints - 1, 1], 0.0).mean(axis=-1)
        ...
        velocity_limit_penalty = jnp.maximum(actuator_joint_abs_velocities - soft_actuator_joint_velocity_limit, 0.0).mean(axis=-1)
        ...
        joint_velocity_norm = jnp.mean(jnp.square(data.qvel[:, self.env.actuator_joint_mask_qvel]), axis=-1)
        joint_velocity_reward = curriculum_coeff * self.joint_velocity_coeff * -joint_velocity_norm

        # Joint acceleration reward
        acceleration_norm = jnp.mean(jnp.square((internal_state["previous_actuator_joint_velocities"] - data.qvel[:, self.env.actuator_joint_mask_qvel]) / self.env.dt), axis=-1)
        acceleration_reward = curriculum_coeff * self.joint_acceleration_coeff * -acceleration_norm

        # Joint torque reward
        joint_torques = data.qfrc_actuator[:, self.env.actuator_joint_mask_qvel]
        joint_velocities = data.qvel[:, self.env.actuator_joint_mask_qvel]
        actuator_forcerange = mjx_model.actuator_forcerange
        if actuator_forcerange.ndim == 2:
            actuator_forcerange = jnp.broadcast_to(actuator_forcerange, (nr_envs,) + actuator_forcerange.shape)
        joint_actuator_force_capacity = jnp.abs(actuator_forcerange).max(axis=-1)
        joint_actuator_force_capacity = jnp.where(joint_actuator_force_capacity > 0.0, joint_actuator_force_capacity, 1.0)
        force_fraction = joint_torques / joint_actuator_force_capacity
        power_fraction = jnp.maximum(joint_torques * joint_velocities, 0.0) / (joint_actuator_force_capacity * internal_state["actuator_joint_max_velocities"])
        torque_norm = jnp.mean(jnp.square(force_fraction), axis=-1)
        torque_reward = curriculum_coeff * self.joint_torque_coeff * -torque_norm

        # Power draw penalty reward
        power_draw = jnp.mean(power_fraction, axis=-1)
        power_draw_penalty_reward = curriculum_coeff * self.power_draw_penalty_coeff * -power_draw

        # Action rate reward
        action_rate_norm = jnp.mean(jnp.square(action - internal_state["last_action"]), axis=-1)
        action_rate_reward = curriculum_coeff * self.action_rate_coeff * -action_rate_norm

        # Action smoothness reward
        action_smoothness_norm = jnp.mean(jnp.square(action - 2 * internal_state["last_action"] + internal_state["second_last_action"]), axis=-1)
        ...
        all_contact_relevant_geom_sizes = geom_size[:, self.env.reward_collision_sphere_geom_ids, 0]
        ...
        ground_penetration = jnp.sum(jnp.maximum(sphere_ground_height + all_contact_relevant_geom_sizes - all_contact_relevant_geom_xpos[..., 2] - internal_state["nr_ground_penetrations_in_nominal"], 0.0), axis=-1)
        ...
        feet_floor_contacts = self.env.terrain_function.check_feet_floor_contact(data)
        is_standing_command = jnp.all(internal_state["goal_velocities"] == 0.0, axis=-1)
        ...
        air_time_reward = jnp.mean(feet_floor_contacts * jnp.minimum(internal_state["feet_time_in_air"] - target_foot_air_time[:, None], 0.0), axis=-1)
        ...
        imu_yaw = internal_state["imu_orientation_euler"][:, 2, None]
        feet_lateral_distances = jnp.abs(-jnp.sin(imu_yaw) * feet_deltas[:, :, 0] + jnp.cos(imu_yaw) * feet_deltas[:, :, 1])
        nominal_feet_lateral_distances = internal_state["nominal_feet_lateral_distances"]
        feet_lateral_distance_deficits = jnp.maximum(self.feet_lateral_min_distance_ratio * nominal_feet_lateral_distances - feet_lateral_distances, 0.0) / jnp.maximum(nominal_feet_lateral_distances, jnp.finfo(jnp.float32).eps)
        feet_lateral_min_distance_penalty = jnp.sum(jnp.square(feet_lateral_distance_deficits), axis=-1) / max(len(self.feet_symmetry_pairs), 1)
        feet_lateral_min_distance_reward = curriculum_coeff * self.feet_lateral_min_distance_coeff * -feet_lateral_min_distance_penalty

        # Symmetry reward
        symmetry_air_violations = jnp.mean(jnp.where((~feet_floor_contacts[:, self.feet_symmetry_pairs[:, 0]]) & (~feet_floor_contacts[:, self.feet_symmetry_pairs[:, 1]]), 1, 0), axis=-1)
        ...
        contact_filtered_feet_slip = jnp.mean(feet_floor_contacts * feet_global_linear_velocity_xy_norm, axis=-1)
        ...
        squared_negative_z_velocity = jnp.mean(jnp.square(jnp.minimum(feet_global_linear_velocity_z, 0.0)), axis=-1)
        ...
        contact_filtered_missing_lower_feet_contacts = jnp.mean(feet_floor_contacts * missing_lower_feet_contacts, axis=-1)
        ...
        feet_rotations = data.xmat[:, self.env.body_ids_of_feet].reshape(nr_envs, -1, 3, 3)
        feet_tilt = jnp.sqrt(jnp.square(feet_rotations[:, :, 2, 0]) + jnp.square(feet_rotations[:, :, 2, 1]) + 1e-12)
        feet_orientation_penalty = jnp.mean(jnp.maximum(feet_tilt - internal_state["nominal_feet_tilt"], 0.0), axis=-1)
        feet_orientation_reward = curriculum_coeff * self.feet_orientation_coeff * -feet_orientation_penalty
        ...
        info[f"reward/ground_penetration"] = ground_penetration_reward
        info[f"reward/base_height"] = base_height_reward
        info[f"reward/foot_air_time"] = foot_air_time_reward
        info[f"reward/feet_lateral_min_distance"] = feet_lateral_min_distance_reward
        ...
        info[f"reward/feet_orientation"] = feet_orientation_reward
        info[f"reward/total"] = reward
        xy_vel_diff_abs = jnp.mean(jnp.minimum(jnp.abs(xy_difference), 2 * internal_state["max_command_velocity"][:, None]), axis=-1)
        info[f"env_info/xy_vel_diff_abs"] = jnp.where(jnp.isfinite(xy_vel_diff_abs), xy_vel_diff_abs, 2 * internal_state["max_command_velocity"])
        info[f"env_info/xy_vel_diff_abs_normalized"] = info[f"env_info/xy_vel_diff_abs"] / jnp.maximum(internal_state["max_command_velocity"], 1e-6)
