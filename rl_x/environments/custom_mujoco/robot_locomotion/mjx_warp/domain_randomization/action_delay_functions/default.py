...
class DefaultActionDelay:
    ...
    def init(self, internal_state):
        internal_state["action_current_mixed"] = jnp.zeros(self.env.nr_envs, dtype=bool)
        internal_state["action_current_nr_delay_steps"] = jnp.zeros(self.env.nr_envs, dtype=jnp.int32)


    def setup(self, internal_state):
        internal_state["action_history"] = jnp.zeros((self.env.nr_envs, self.max_nr_delay_steps + 1, self.env.nr_actuator_joints))


    def sample(self, internal_state, should_randomize, key):
        internal_state["action_current_mixed"] = jnp.where(should_randomize, jax.random.uniform(key, (self.env.nr_envs,)) < self.mixed_chance, internal_state["action_current_mixed"])
    ...
    def delay_action(self, action, internal_state, key):
        ...
        current_nr_delay_steps = jnp.ceil(jnp.where(
            ...
            jax.random.randint(key, (nr_envs,), 0, self.max_nr_delay_steps + 1),
            internal_state["action_current_nr_delay_steps"]
        ) * internal_state["env_curriculum_coeff"]).astype(jnp.int32)

        internal_state["action_history"] = jnp.roll(internal_state["action_history"], -1, axis=1)
        internal_state["action_history"] = internal_state["action_history"].at[:, -1].set(action)

        chosen_action = internal_state["action_history"][jnp.arange(nr_envs), -1 - current_nr_delay_steps]
