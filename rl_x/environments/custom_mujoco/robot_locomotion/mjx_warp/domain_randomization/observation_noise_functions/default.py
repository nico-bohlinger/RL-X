...
class DefaultDRObservationNoise:
    ...
    def modify_observation(self, internal_state, observation, key):
        ...
        observation = observation.at[:, self.joint_velocities_obs_idx].set(observation[:, self.joint_velocities_obs_idx] + curriculum_coeff * jax.random.uniform(noise_key3, shape=(nr_envs, len(self.joint_velocities_obs_idx)), minval=-self.joint_velocity_noise, maxval=self.joint_velocity_noise))
        observation = observation.at[:, self.gravity_vector_obs_idx].set(observation[:, self.gravity_vector_obs_idx] + curriculum_coeff * jax.random.uniform(noise_key4, shape=(nr_envs, len(self.gravity_vector_obs_idx)), minval=-self.gravity_vector_noise, maxval=self.gravity_vector_noise))
        if len(self.policy_exteroception_obs_idx) > 0:
            observation = observation.at[:, self.policy_exteroception_obs_idx].set(observation[:, self.policy_exteroception_obs_idx] + curriculum_coeff * jax.random.uniform(noise_key5, shape=(nr_envs, len(self.policy_exteroception_obs_idx)), minval=-self.exteroception_noise, maxval=self.exteroception_noise))
