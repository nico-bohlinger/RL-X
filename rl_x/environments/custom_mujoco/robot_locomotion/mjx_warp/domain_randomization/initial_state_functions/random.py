...
class RandomDRInitialState:
    ...
    def setup(self, mjx_model, internal_state, key):
        ...
        min_feet_z_pos_under_ground = jnp.max(self.env.terrain_function.ground_height_at(internal_state, feet_x_pos, feet_y_pos) - (data.geom_xpos[:, self.env.foot_geom_indices, 2] - self.env.feet_bottom_extent(data, mjx_model)), axis=-1)
