...
class DepthImageExteroceptiveObservation:
    def __init__(self, env, fov_y=45, p_x=16, p_y=16, f=0.1, min_depth=0.28, max_depth=10.0):
        ...
        if self.floor_is_plane:
            ...
            self.hfield_nrow = int(self.env.initial_mjx_model.hfield_nrow[0])
            self.hfield_ncol = int(self.env.initial_mjx_model.hfield_ncol[0])
    ...
    def get_exteroceptive_observation(self, data, mjx_model, internal_state):
        cam_xpos = data.cam_xpos[:, self.camera_id]
        cam_xmat = data.cam_xmat[:, self.camera_id].reshape(self.env.nr_envs, 3, 3)
        hfield_data = mjx_model.hfield_data
        return jax.vmap(self.compute_depth_image, in_axes=(0, 0, None))(cam_xpos, cam_xmat, hfield_data)


    def compute_depth_image(self, ray_origin, cam_bases, hfield_data):
        ...
        if self.floor_is_plane:
            ...
            h_vals = hfield_data.reshape((self.hfield_nrow, self.hfield_ncol))[j0, i0] * self.hfield_size[2]
