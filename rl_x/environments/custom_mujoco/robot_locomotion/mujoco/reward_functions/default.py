...
class DefaultReward:
    ...
    def reward_and_info(self, action):
        ...
        feet_rotations = self.env.internal_state["data"].xmat[self.env.body_ids_of_feet].reshape(-1, 3, 3)
        feet_tilt = np.sqrt(np.square(feet_rotations[:, 2, 0]) + np.square(feet_rotations[:, 2, 1]) + 1e-12)
        feet_orientation_penalty = np.mean(np.maximum(feet_tilt - self.env.internal_state["nominal_feet_tilt"], 0.0))
        ...
        self.env.internal_state["info"][f"reward/feet_orientation"] = feet_orientation_reward
