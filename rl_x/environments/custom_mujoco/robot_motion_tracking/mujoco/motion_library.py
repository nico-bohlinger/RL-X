    ...
    def __init__(self, env_config, mj_model):
        ...
        for file in files:
            ...
            if stored_reference is not None:
                ...
                root_rotation = Rotation.from_quat(root_quat[:, self.quaternion_wxyz_to_xyzw_indices])
        ...
        self.arrays = {key: np.asarray(np.concatenate([a[key] for a in arrays]), dtype=np.float32) for key in arrays[0]}


    def sample(self, clip, time):
        frame = np.clip(time * self.fps[clip], 0, self.lengths[clip] - 1)
        frame_index = frame.astype(np.int32)
        left = self.starts[clip] + frame_index
        right = self.starts[clip] + np.minimum(frame_index + 1, self.lengths[clip] - 1)
        ...
        for name, array in self.arrays.items():
            ...
            weight = np.reshape(alpha, alpha.shape + (1,) * (left_value.ndim - alpha.ndim))
        ...
        for name, start, stop in quaternion_fields:
            ...
            weight = np.reshape(alpha, alpha.shape + (1,) * (a.ndim - 1 - alpha.ndim))
            weight = np.broadcast_to(weight, a.shape[:-1]).reshape(-1, 1)
            ...
            result[name][..., start:stop] = quaternion
