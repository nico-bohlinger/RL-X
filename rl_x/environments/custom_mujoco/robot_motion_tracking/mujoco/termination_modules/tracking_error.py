...
class TrackingErrorTermination:
    ...
    def should_terminate(self):
        anchor_height_difference = np.abs(self.env.internal_state["anchor_position"][..., 2] - self.env.internal_state["reference"]["anchor_position"][..., 2])
        anchor_height_failure = anchor_height_difference > self.anchor_height_threshold

        reference_gravity = self.env.internal_state["reference"]["anchor_rotation"].inv().apply([0.0, 0.0, -1.0])
        anchor_orientation_difference = np.abs(self.env.internal_state["projected_gravity"][..., 2] - reference_gravity[..., 2])
        anchor_orientation_failure = anchor_orientation_difference > self.anchor_orientation_threshold

        body_height_difference = np.abs(self.env.internal_state["body_positions"][..., 2] - self.env.internal_state["reference"]["aligned_body_positions"][..., 2])
        body_height_failure = np.any(body_height_difference[..., self.body_indices] > self.body_height_threshold, axis=-1)

        terminated = anchor_height_failure | anchor_orientation_failure | body_height_failure
        if self.env.motion_library.has_object:
            object_position_error_squared = np.sum(np.square(self.env.internal_state["reference"]["qpos"][..., self.env.object_position_qpos_slice] - self.env.internal_state["data"].qpos[..., self.env.object_position_qpos_slice]), axis=-1)
        ...
        self.env.internal_state["info"]["termination/anchor_height"] = anchor_height_failure
        self.env.internal_state["info"]["termination/anchor_orientation"] = anchor_orientation_failure
        self.env.internal_state["info"]["termination/body_height"] = body_height_failure
        if self.env.motion_library.has_object:
            self.env.internal_state["info"]["termination/object_position"] = object_position_failure
            self.env.internal_state["info"]["termination/object_orientation"] = object_orientation_failure
