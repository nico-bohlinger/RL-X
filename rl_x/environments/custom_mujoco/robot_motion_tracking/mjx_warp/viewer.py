...
class MujocoViewer:
    def __init__(self, model, dt):
        ...
        self.reference_perturbation = mujoco.MjvPerturb()
    ...
    def render(self, data, reference_qpos):
        self.reference_data.qpos[:] = reference_qpos
        ...
        def render_inner_loop(self):
            ...
            if self.show_reference:
                ...
                nr_simulation_geoms = self.scene.ngeom
                mujoco.mjv_addGeoms(self.model, self.reference_data, self.scene_option, self.reference_perturbation, mujoco.mjtCatBit.mjCAT_DYNAMIC, self.scene)
                for geom in self.scene.geoms[nr_simulation_geoms:self.scene.ngeom]:
                    geom.matid = -1
                    geom.rgba[:] = [0.2, 0.6, 1.0, 0.35]
    ...
    def create_overlay(self):
        ...
        self.overlay[topleft][1] += "\n"
        self.overlay[topleft][0] += "Press R to toggle the reference.\n"
        self.overlay[topleft][1] += "On\n" if self.show_reference else "Off\n"
    ...
    def set_camera(self):
        ...
        if self.camera_mode_target == "follow" and self.camera_mode != "follow":
            ...
            self.camera.trackbodyid = self.trunk_body_id
