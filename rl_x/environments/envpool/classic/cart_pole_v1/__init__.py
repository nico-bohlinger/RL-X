from rl_x.environments.environment_manager import extract_environment_name_from_file, register_environment
from rl_x.environments.envpool.classic.cart_pole_v1.create_env import create_train_and_eval_env
from rl_x.environments.envpool.classic.cart_pole_v1.default_config import get_config
from rl_x.environments.envpool.classic.cart_pole_v1.general_properties import GeneralProperties


ENVPOOL_CLASSIC_CART_POLE_V1 = extract_environment_name_from_file(__file__)
register_environment(ENVPOOL_CLASSIC_CART_POLE_V1, get_config, create_train_and_eval_env, GeneralProperties)
