...
from rl_x.algorithms.tqc_per.flax.policy import get_policy
from rl_x.algorithms.tqc_per.flax.critic import get_critic
from rl_x.algorithms.tqc_per.flax.entropy_coefficient import EntropyCoefficient, ConstantEntropyCoefficient
from rl_x.algorithms.tqc_per.flax.replay_buffer import ReplayBuffer
from rl_x.algorithms.tqc_per.flax.rl_train_state import RLTrainState
...
class TQC_PER():
    def __init__(self, config, env, run_path, writer) -> None:
        ...
        if self.nr_envs > 1:
            raise NotImplementedError("A replay buffer + priority tree for nr_envs > 1 is not implemented")
    ...
    def train(self):
        ...
        def update_critics(
                ...
                states: jnp.ndarray, next_states: jnp.ndarray, actions: jnp.ndarray, rewards: jnp.ndarray, dones: jnp.ndarray, batch_is_weights: jnp.ndarray, key: jax.random.PRNGKey
            ):
            q_losses = []
            ...
            for i in range(self.q_update_steps):
                ...
                def huber_loss_fn(vector_critic_params: flax.core.FrozenDict):
                    ...
                    loss *= batch_is_weights[i].reshape(-1, 1, 1)

                    return loss.mean(), delta_i_j.mean(axis=(1,2))
                
                (q_loss, delta), grads = jax.value_and_grad(huber_loss_fn, has_aux=True)(vector_critic_state.params)
            ...
            return jnp.array(q_losses), delta, vector_critic_state, key


        @jax.jit
        def update_policy(policy_state: TrainState, vector_critic_state: RLTrainState, entropy_coefficient_state: TrainState, states: jnp.ndarray, batch_is_weights: jnp.ndarray, key: jax.random.PRNGKey):
            policy_losses = []
            entropies = []
            alphas = []
            ...
            for i in range(self.policy_update_steps):
                ...
                def loss_fn(policy_params: flax.core.FrozenDict):
                    ...
                    policy_loss = (alpha * current_log_probs - mean_q_atoms)
                    policy_loss *= batch_is_weights[i].reshape(-1, 1, 1)

                    return policy_loss.mean(), (-current_log_probs.mean(), alpha)
        ...
        while global_step < self.total_timesteps:
            ...
            if should_update_q or should_update_policy or should_update_entropy:
                ...
                batch_states, batch_next_states, batch_actions, batch_rewards, batch_dones, batch_is_weights, batch_buffer_idx = replay_buffer.sample(self.batch_size, max_nr_batches_needed)

            
            # Optimizing - Q-functions
            if should_update_q:
                q_losses, delta, self.vector_critic_state, self.key = update_critics(
                        self.policy_state, self.vector_critic_state, self.entropy_coefficient_state,
                        batch_states, batch_next_states, batch_actions, batch_rewards, batch_dones, batch_is_weights, self.key
                )
                replay_buffer.update_priorities(batch_buffer_idx, delta)
            ...
            if should_update_policy:
                policy_losses, batch_entropies, alphas, self.policy_state, self.key = update_policy(
                        self.policy_state, self.vector_critic_state, self.entropy_coefficient_state,
                        batch_states, batch_is_weights, self.key
                )
    ...
    def load(config, env, run_path, writer):
        ...
        model = TQC_PER(config, env, run_path, writer)
