...
from rl_x.algorithms.aqe.pytorch.policy import get_policy
from rl_x.algorithms.aqe.pytorch.critic import get_critic
from rl_x.algorithms.aqe.pytorch.entropy_coefficient import get_entropy_coefficient
from rl_x.algorithms.aqe.pytorch.replay_buffer import ReplayBuffer
...
class AQE:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        if self.anneal_learning_rate:
            ...
            self.q_scheduler = optim.lr_scheduler.LinearLR(self.q_optimizer, start_factor=1.0, end_factor=0.0, total_iters=total_iterations * self.q_update_steps)
            ...
            self.entropy_scheduler = optim.lr_scheduler.LinearLR(self.entropy_optimizer, start_factor=1.0, end_factor=0.0, total_iters=total_iterations)
    ...
    def train(self):
        ...
        def policy_and_entropy_loss_fn(batch_states):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                ...
                mean_q = self.critic.q(batch_states, current_actions).mean(dim=(0, 2)).reshape(-1, 1)

                alpha = self.entropy_coefficient()
                alpha_detach = alpha.detach()
                policy_loss = (alpha_detach * current_log_probs - mean_q).mean()
            ...
            return policy_loss, entropy_loss, mean_q, entropy_detach_mean, alpha_detach, policy_grad_norm, entropy_grad_norm
        ...
        def critic_loss_fn(states, next_states, actions, rewards, dones):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                with torch.no_grad():
                    ...
                    next_q_target = self.critic.q_target(next_states, next_actions).permute(1, 0, 2).reshape(next_states.shape[0], self.nr_total_q_values)
                    next_q_target = next_q_target.sort(dim=-1).values[:, :self.nr_target_q_values].mean(dim=-1, keepdim=True)
                    alpha = self.entropy_coefficient().detach()
                    y = rewards.reshape(-1, 1) + self.gamma * (1 - dones.reshape(-1, 1)) * (next_q_target - alpha * next_log_probs)
            ...
            critic_grad_norm = torch.nn.utils.clip_grad_norm_(self.critic.q.parameters(), float("inf"))
        ...
        while global_step < self.total_timesteps:
            ...
            if should_optimize:
                ...
                for q_update_step in range(self.q_update_steps):
                    q_loss, critic_grad_norm = critic_loss_fn(batch_states[q_update_step], batch_next_states[q_update_step], batch_actions[q_update_step], batch_rewards[q_update_step], batch_terminations[q_update_step])
                    q_losses.append(q_loss.item())
                    critic_grad_norms.append(critic_grad_norm.item())
                    with torch.no_grad():
                        for param, target_param in zip(self.critic.q.parameters(), self.critic.q_target.parameters()):
                            target_param.data.mul_(1.0 - self.tau).add_(param.data, alpha=self.tau)
                    if self.anneal_learning_rate:
                        self.q_scheduler.step()

                # Policy and entropy loss
                policy_loss, entropy_loss, mean_q, entropy, alpha, policy_grad_norm, entropy_grad_norm = policy_and_entropy_loss_fn(batch_states[0])
                ...
                optimization_metrics = {
                    ...
                    "lr/learning_rate": self.learning_rate if not self.anneal_learning_rate else self.policy_scheduler.get_last_lr()[0],
                    "q_value/q_value": mean_q.mean().item(),
                }
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = AQE(config, train_env, eval_env, run_path, writer)
