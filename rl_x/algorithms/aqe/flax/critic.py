...
def get_critic(config, env):
    ...
    if observation_space_type == ObservationSpaceType.FLAT_VALUES:
        return VectorCritic(config.algorithm.nr_hidden_units, config.algorithm.nr_heads_per_net, config.algorithm.ensemble_size)
...
class Critic(nn.Module):
    ...
    def __call__(self, x: np.ndarray, a: np.ndarray):
        ...
        x = nn.Dense(self.nr_hidden_units)(x)
        x = nn.relu(x)
        x = nn.Dense(self.nr_heads_per_net)(x)
        return x
...
class VectorCritic(nn.Module):
    ...
    def __call__(self, obs: np.ndarray, action: np.ndarray):
        ...
        q_values = vmap_critic(nr_hidden_units=self.nr_hidden_units, nr_heads_per_net=self.nr_heads_per_net)(obs, action)
        return q_values
