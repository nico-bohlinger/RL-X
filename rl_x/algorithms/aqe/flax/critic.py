...
def get_critic(config, env):
    ...
    if observation_space_type == ObservationSpaceType.FLAT_VALUES:
        return VectorCritic(config.algorithm.nr_hidden_units, config.algorithm.nr_heads_per_net, config.algorithm.ensemble_size, critic_observation_indices)
...
class VectorCritic(nn.Module):
    ...
    def __call__(self, obs: np.ndarray, action: np.ndarray):
        ...
        q_values = vmap_critic(
            ...
            nr_heads_per_net=self.nr_heads_per_net,
