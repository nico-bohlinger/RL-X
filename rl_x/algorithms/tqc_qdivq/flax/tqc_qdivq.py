...
from rl_x.algorithms.tqc_qdivq.flax.policy import get_policy
from rl_x.algorithms.tqc_qdivq.flax.critic import get_critic
from rl_x.algorithms.tqc_qdivq.flax.entropy_coefficient import EntropyCoefficient, ConstantEntropyCoefficient
from rl_x.algorithms.tqc_qdivq.flax.replay_buffer import ReplayBuffer
from rl_x.algorithms.tqc_qdivq.flax.rl_train_state import RLTrainState
...
class TQC_QdivQ():
    ...
    def train(self):
        ...
        def update(
            ...
            for i in range(self.q_update_steps):
                ...
                traces_ = self.q_lambda * jnp.clip(next_q_target_atoms[:, 1:, :] / current_q_target_atoms[:, :-1, :], 0, 1)  # (batch_size, trace_length - 1, nr_target_atoms)
    ...
    def load(config, env, run_path, writer):
        ...
        model = TQC_QdivQ(config, env, run_path, writer)
