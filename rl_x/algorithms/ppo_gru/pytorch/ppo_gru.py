...
from rl_x.algorithms.ppo_gru.pytorch.policy import get_policy
from rl_x.algorithms.ppo_gru.pytorch.critic import get_critic
from rl_x.algorithms.ppo_gru.pytorch.batch import Batch
...
class PPO_GRU:
    ...
    def train(self):
        ...
        batch = Batch(
            ...
            init_policy_carry=torch.zeros((self.nr_envs, self.gru_hidden_dim), dtype=torch.float32, device=self.device),
        )
        ...
        while global_step < self.total_timesteps:
            ...
            with torch.inference_mode():
                ...
                batch.init_policy_carry.copy_(policy_carry)
                for step in range(self.nr_steps):
                    ...
                    state = next_state
                    policy_carry = next_policy_carry * (1 - batch.dones[step].unsqueeze(-1))
            ...
            for epoch in range(self.nr_epochs):
                ...
                for start in range(0, self.nr_envs, self.nr_minibatch_envs):
                    ...
                    pg_loss, entropy_loss, approx_kl_div, clip_fraction, policy_grad_norm = policy_loss_fn(batch.states[:, minibatch_env_indices], batch.actions[:, minibatch_env_indices], batch.log_probs[:, minibatch_env_indices], batch.advantages[:, minibatch_env_indices], batch.dones[:, minibatch_env_indices], batch.init_policy_carry[minibatch_env_indices])
            ...
            if global_step % self.evaluation_frequency == 0 and self.evaluation_frequency != -1:
                with torch.inference_mode():
                    ...
                    while True:
                        ...
                        eval_policy_carry = next_eval_policy_carry * (1 - torch.tensor(eval_done, dtype=torch.float32, device=self.device).unsqueeze(-1))
        ...
        with torch.inference_mode():
            ...
            for episode in range(episodes):
                ...
                while not done:
                    ...
                    state = torch.tensor(state, dtype=torch.float32, device=self.device)
                    policy_carry = next_policy_carry * (1 - torch.tensor(done, dtype=torch.float32, device=self.device).unsqueeze(-1))
