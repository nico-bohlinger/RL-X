...
class DroQ:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.critic_state = RLTrainState.create(
            ...
            params=self.critic.init({"params": critic_key, "dropout": dropout_key}, state, action),
            target_params=self.critic.init({"params": critic_key, "dropout": dropout_key}, state, action),
    ...
    def train(self):
        ...
        def update(
            ...
            def critic_loss_fn(critic_params: flax.core.FrozenDict, m_target_params,
                        ...
                        sample_key: jax.random.PRNGKey, dropout_current_key: jax.random.PRNGKey, dropout_target_key: jax.random.PRNGKey
                ):
                # Critic loss
                ...
                next_action_pretanh = next_action_mean + next_action_std * jax.random.normal(sample_key, shape=next_action_mean.shape)
                ...
                next_q_target = self.critic.apply(m_target_params, next_state, next_action, rngs={"dropout": dropout_target_key})
                min_next_q_target = jnp.min(next_q_target)
                y = reward + self.gamma * (1 - terminated) * (min_next_q_target - alpha * next_log_prob)

                q = self.critic.apply(critic_params, state, action, rngs={"dropout": dropout_current_key})
            ...
            def policy_entropy_loss_fn(policy_params: flax.core.FrozenDict, entropy_coefficient_params: flax.core.FrozenDict,
                        state: np.ndarray, sample_key: jax.random.PRNGKey, dropout_key: jax.random.PRNGKey
                ):
                # Policy loss
                ...
                current_action_pretanh = current_action_mean + current_action_std * jax.random.normal(sample_key, shape=current_action_mean.shape)
                ...
                q = self.critic.apply(critic_state.params, state, current_action, rngs={"dropout": dropout_key})
                min_q = jnp.min(q)
            ...
            vmap_critic_loss_fn = jax.vmap(critic_loss_fn, in_axes=(None, None, 0, 0, 0, 0, 0, 0, 0, 0), out_axes=0)
            vmap_policy_entropy_loss_fn = jax.vmap(policy_entropy_loss_fn, in_axes=(None, None, 0, 0, 0), out_axes=0)
            ...
            for i in range(self.q_update_steps):
                keys = jax.random.split(key, (self.batch_size * 3) + 2)  # Using a single key for all vmap calls would be faster
                key, sample_key, keys1, keys2, keys3 = keys[0], keys[1], keys[2::3], keys[3::3], keys[4::3]
                ...
                (loss, (metrics)), (critic_gradients) = grad_critic_loss_fn(critic_state.params, m_target_params, states[i], next_states[i], actions[i], rewards[i], terminations[i], keys1, keys2, keys3)
            ...
            (loss, (policy_entropy_metrics)), (policy_gradients, entropy_gradients) = grad_policy_entropy_loss_fn(policy_state.params, entropy_coefficient_state.params, states[0], keys1, keys2)
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = DroQ(config, train_env, eval_env, run_path, writer)
