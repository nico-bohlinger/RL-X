...
from rl_x.algorithms.droq.flax.entropy_coefficient import EntropyCoefficient, ConstantEntropyCoefficient
    ...
    def train(self):
        ...
        def update_critics(
            ...
            for i in range(self.q_update_steps):
                key, action_sample_key, critic_sample_key, dropout_current_key, dropout_target_key = jax.random.split(key, num=5)
                ...
                next_q_targets = self.vector_critic.apply(m_target_params, next_states[i], next_actions, rngs={"dropout": dropout_target_key})
                ...
                def loss_fn(vector_critic_params: flax.core.FrozenDict):
                    qs = self.vector_critic.apply(vector_critic_params, states[i], actions[i], rngs={"dropout": dropout_current_key})
                    return ((qs - y) ** 2).mean()
        ...
        def update_policy(policy_state: TrainState, vector_critic_state: RLTrainState, entropy_coefficient_state: TrainState, states: jnp.ndarray, key: jax.random.PRNGKey):
            ...
            for i in range(self.policy_update_steps):
                ...
                def loss_fn(policy_params: flax.core.FrozenDict):
                    ...
                    qs = self.vector_critic.apply(vector_critic_state.params, states[i], current_actions, rngs={"dropout": dropout_key})
