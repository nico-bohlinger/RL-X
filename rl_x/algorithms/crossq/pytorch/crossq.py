...
from rl_x.algorithms.crossq.pytorch.entropy_coefficient import get_entropy_coefficient
...
class CrossQ:
    ...
    def train(self):
        ...
        def policy_and_entropy_loss_fn(batch_states):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                current_actions, _, current_log_probs = self.policy.get_action(batch_states, True)
                q_values = self.critic(batch_states, current_actions, False).squeeze(-1)
        ...
        def critic_loss_fn(states, next_states, actions, rewards, terminations):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                ...
                current_and_next_q = self.critic(torch.cat([states, next_states], dim=0), torch.cat([actions, next_actions], dim=0), True).squeeze(-1)
                q, next_q = torch.split(current_and_next_q, states.shape[0], dim=1)
                min_next_q = torch.min(next_q.detach(), dim=0).values
                y = rewards + self.gamma * (1.0 - terminations) * (min_next_q - self.entropy_coefficient().detach() * next_log_probs)
        ...
        while global_step < self.total_timesteps:
            ...
            if should_optimize_critic:
                critic_loss, critic_grad_norm = critic_loss_fn(batch_states, batch_next_states, batch_actions, batch_rewards, batch_terminations)
                optimization_metrics = {
                    "gradients/critic_grad_norm": critic_grad_norm.item(),
                    "loss/q_loss": critic_loss.item(),
                    "lr/learning_rate": self.critic_optimizer.param_groups[0]["lr"],
                }
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = CrossQ(config, train_env, eval_env, run_path, writer)
