...
def get_critic(config, env):
    ...
    if observation_space_type == ObservationSpaceType.FLAT_VALUES:
        return VectorCritic(config.algorithm.nr_atoms_per_net, config.algorithm.nr_hidden_units, config.algorithm.dropout_rate, config.algorithm.ensemble_size)
    else:
...
class VectorCritic(nn.Module):
    ...
    def __call__(self, obs: jnp.ndarray, action: jnp.ndarray):
        ...
        q_values = vmap_critic(nr_atoms=self.nr_atoms_per_net, nr_hidden_units=self.nr_hidden_units, dropout_rate=self.dropout_rate)(obs, action)
        return q_values
