...
def get_policy(config, env):
    ...
    if action_space_type == ActionSpaceType.CONTINUOUS and observation_space_type == ObservationSpaceType.FLAT_VALUES:
        return (Policy(
                    ...
                    config.algorithm.lstm_hidden_dim,
                    config.algorithm.lstm_obs_combine_method,
                    config.algorithm.share_lstm_obs_encoder,
...
class Policy(nn.Module):
    ...
    lstm_hidden_dim: int
    lstm_obs_combine_method: str
    share_lstm_obs_encoder: bool
    policy_observation_indices: Sequence[int]

    def setup(self):
        act_dim = int(np.prod(self.as_shape).item())

        self.lstm_obs_encoder_dense = nn.Dense(self.obs_encoding_dim, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))
        self.lstm_obs_encoder_ln = nn.LayerNorm()
        ...
        self.lstm = nn.OptimizedLSTMCell(features=self.lstm_hidden_dim)
        self.lstm_ln = nn.LayerNorm()

        if self.lstm_obs_combine_method == "film":
            self.lstm_film_gamma = nn.Dense(self.obs_encoding_dim, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))
            self.lstm_film_beta = nn.Dense(self.obs_encoding_dim, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))
    ...
    def initialize_carry(self, nr_envs: int):
        return (jnp.zeros((nr_envs, self.lstm_hidden_dim), dtype=jnp.float32),
                jnp.zeros((nr_envs, self.lstm_hidden_dim), dtype=jnp.float32))
    ...
    def lstm_obs_encode(self, obs):
        x = obs[..., self.policy_observation_indices]
        x = self.lstm_obs_encoder_dense(x)
        x = self.lstm_obs_encoder_ln(x)
        x = nn.tanh(x)
        return x


    def decode(self, obs_latent, lstm_latent):
        lstm_latent = self.lstm_ln(lstm_latent)
        lstm_latent = nn.tanh(lstm_latent)

        if self.lstm_obs_combine_method == "concat":
            torso_in = jnp.concatenate([obs_latent, lstm_latent], axis=-1)
        elif self.lstm_obs_combine_method == "film":
            gamma = self.lstm_film_gamma(lstm_latent)
            beta = self.lstm_film_beta(lstm_latent)
    ...
    def apply_one_step(self, obs, carry):
        lstm_obs_latent = self.lstm_obs_encode(obs)
        carry, hidden = self.lstm(carry, lstm_obs_latent)
    ...
    def forward_sequence(self, obs_seq, done_seq, init_carry):
        ...
        init_carry: ([H], [H]) carry valid for obs_seq[0]
        """
        ...
        def step(carry, inp):
            ...
            carry = jax.tree_util.tree_map(lambda c: c * (1.0 - done_prev_t), carry)
