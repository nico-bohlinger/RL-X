...
from rl_x.algorithms.ppo_lstm.flax_full_jit.policy import get_policy
from rl_x.algorithms.ppo_lstm.flax_full_jit.critic import get_critic
...
class PPO_LSTM:
    ...
    def train(self):
        def jitable_train_function(key, parallel_seed_id):
            ...
            policy_lstm_carry = self.policy.initialize_carry(self.nr_envs)

            def multi_learning_and_eval_save_iteration(multi_learning_and_eval_save_iteration_carry, multi_learning_iteration_step):
                policy_state, critic_state, env_state, policy_lstm_carry, key = multi_learning_and_eval_save_iteration_carry

                def learning_iteration(learning_iteration_carry, learning_iteration_step):
                    policy_state, critic_state, env_state, policy_lstm_carry, key = learning_iteration_carry

                    # Acting
                    def single_rollout(single_rollout_carry, _):
                        policy_state, critic_state, env_state, policy_lstm_carry, key = single_rollout_carry

                        key, subkey = jax.random.split(key)
                        observation = env_state.next_observation
                        action_mean, action_logstd, next_policy_lstm_carry = self.policy.apply(policy_state.params, observation, policy_lstm_carry, method=self.policy.apply_one_step)
                        ...
                        next_policy_lstm_carry = jax.tree_util.tree_map(lambda c: c * (1.0 - done[:, None]), next_policy_lstm_carry)
                        ...
                        return (policy_state, critic_state, env_state, next_policy_lstm_carry, key), transition

                    rollout_init_policy_carry = policy_lstm_carry
                    single_rollout_carry, batch = jax.lax.scan(single_rollout, (policy_state, critic_state, env_state, policy_lstm_carry, key), None, self.nr_steps)
                    policy_state, critic_state, env_state, policy_lstm_carry, key = single_rollout_carry
                    ...
                    def minibatch_update(carry, minibatch_env_indices):
                        ...
                        (loss, (metrics)), (policy_gradients, critic_gradients) = grad_loss_fn(
                            ...
                            jax.tree_util.tree_map(lambda c: c[minibatch_env_indices], rollout_init_policy_carry),
                        )
                    ...
                    return (policy_state, critic_state, env_state, policy_lstm_carry, key), None

                key, subkey = jax.random.split(key)
                learning_iteration_carry, _ = jax.lax.scan(learning_iteration, (policy_state, critic_state, env_state, policy_lstm_carry, subkey), jnp.arange(self.nr_updates_per_multi_learning_iteration))
                policy_state, critic_state, env_state, policy_lstm_carry, key = learning_iteration_carry
                ...
                if self.evaluation_active:
                    def single_eval_rollout(single_eval_rollout_carry, _):
                        policy_state, eval_env_state, eval_policy_lstm_carry = single_eval_rollout_carry

                        eval_action_mean, _, eval_policy_lstm_carry = self.policy.apply(policy_state.params, eval_env_state.next_observation, eval_policy_lstm_carry, method=self.policy.apply_one_step)
                        ...
                        eval_policy_lstm_carry = jax.tree_util.tree_map(lambda c: c * (1.0 - eval_done[:, None]), eval_policy_lstm_carry)

                        return (policy_state, eval_env_state, eval_policy_lstm_carry), None

                    key, reset_key = jax.random.split(key)
                    reset_keys = jax.random.split(reset_key, self.nr_envs)
                    eval_env_state = self.eval_env.reset(reset_keys, True)
                    eval_policy_lstm_carry = self.policy.initialize_carry(self.nr_envs)
                    single_eval_rollout_carry, _ = jax.lax.scan(single_eval_rollout, (policy_state, eval_env_state, eval_policy_lstm_carry), jnp.arange(self.horizon))
            ...
            jax.lax.scan(multi_learning_and_eval_save_iteration, (policy_state, critic_state, env_state, policy_lstm_carry, key), jnp.arange(self.nr_multi_learning_and_eval_save_iterations))
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = PPO_LSTM(config, train_env, eval_env, run_path, writer)
    ...
    def test(self, episodes):
        ...
        @jax.jit
        def rollout(env_state, policy_lstm_carry, key):
            # key, subkey = jax.random.split(key)
            action_mean, action_logstd, policy_lstm_carry = self.policy.apply(self.policy_state.params, env_state.next_observation, policy_lstm_carry, method=self.policy.apply_one_step)
            # action_std = jnp.exp(action_logstd)
            action = action_mean # + action_std * jax.random.normal(subkey, shape=action_mean.shape)
            processed_action = self.get_processed_action(action)
            env_state = self.train_env.step(env_state, processed_action)
            done = env_state.terminated | env_state.truncated
            policy_lstm_carry = jax.tree_util.tree_map(lambda c: c * (1.0 - done[:, None]), policy_lstm_carry)
            return env_state, policy_lstm_carry, key
        ...
        policy_lstm_carry = self.policy.initialize_carry(self.nr_envs)
        while True:
            env_state, policy_lstm_carry, self.key = rollout(env_state, policy_lstm_carry, self.key)
            if self.render:
