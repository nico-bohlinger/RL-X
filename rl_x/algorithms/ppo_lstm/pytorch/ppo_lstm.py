...
from rl_x.algorithms.ppo_lstm.pytorch.policy import get_policy
from rl_x.algorithms.ppo_lstm.pytorch.critic import get_critic
from rl_x.algorithms.ppo_lstm.pytorch.batch import Batch
...
class PPO_LSTM:
    ...
    def train(self):
        ...
        batch = Batch(
            ...
            init_policy_carry=(torch.zeros((self.nr_envs, self.lstm_hidden_dim), dtype=torch.float32, device=self.device), torch.zeros((self.nr_envs, self.lstm_hidden_dim), dtype=torch.float32, device=self.device)),
        )

        saving_return_buffer = deque(maxlen=100 * self.nr_envs)
        state, _ = self.train_env.reset()
        state = torch.tensor(state, dtype=torch.float32, device=self.device)
        policy_carry = self.policy.initialize_carry(self.nr_envs)
        global_step = 0
        nr_updates = 0
        nr_episodes = 0
        steps_metrics = {}
        ...
        while global_step < self.total_timesteps:
            ...
            with torch.inference_mode():
                ...
                for batch_carry, carry in zip(batch.init_policy_carry, policy_carry):
                    batch_carry.copy_(carry)
                for step in range(self.nr_steps):
                    torch.compiler.cudagraph_mark_step_begin()
                    with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        action, processed_action, log_prob, next_policy_carry = self.policy.get_action_logprob(state, policy_carry)
                    ...
                    state = next_state
                    policy_carry = tuple(carry * (1 - batch.dones[step].unsqueeze(-1)) for carry in next_policy_carry)
            ...
            for epoch in range(self.nr_epochs):
                ...
                for start in range(0, self.nr_envs, self.nr_minibatch_envs):
                    ...
                    pg_loss, entropy_loss, approx_kl_div, clip_fraction, policy_grad_norm = policy_loss_fn(batch.states[:, minibatch_env_indices], batch.actions[:, minibatch_env_indices], batch.log_probs[:, minibatch_env_indices], batch.advantages[:, minibatch_env_indices], batch.dones[:, minibatch_env_indices], tuple(carry[minibatch_env_indices] for carry in batch.init_policy_carry))
            ...
            if global_step % self.evaluation_frequency == 0 and self.evaluation_frequency != -1:
                with torch.inference_mode():
                    ...
                    eval_policy_carry = self.policy.initialize_carry(eval_state.shape[0])
                    eval_nr_episodes = 0
                    evaluation_metrics = {"eval/episode_return": [], "eval/episode_length": []}
                    while True:
                        torch.compiler.cudagraph_mark_step_begin()
                        with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                            eval_processed_action, next_eval_policy_carry = self.policy.get_deterministic_action(eval_state, eval_policy_carry)
                        ...
                        eval_policy_carry = tuple(carry * (1 - torch.tensor(eval_done, dtype=torch.float32, device=self.device).unsqueeze(-1)) for carry in next_eval_policy_carry)
    ...
    def test(self, episodes):
        with torch.inference_mode():
            ...
            for episode in range(episodes):
                ...
                state = torch.tensor(state, dtype=torch.float32, device=self.device)
                policy_carry = self.policy.initialize_carry(state.shape[0])
                while not done:
                    torch.compiler.cudagraph_mark_step_begin()
                    with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        processed_action, next_policy_carry = self.policy.get_deterministic_action(state, policy_carry)
                    state, reward, terminated, truncated, info = self.eval_env.step(processed_action.float().cpu().numpy())
                    done = terminated | truncated
                    state = torch.tensor(state, dtype=torch.float32, device=self.device)
                    policy_carry = tuple(carry * (1 - torch.tensor(done, dtype=torch.float32, device=self.device).unsqueeze(-1)) for carry in next_policy_carry)
