...
class Critic(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray):
        ...
        x = nn.Dense(self.nr_hidden_units, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))(x)
        x = nn.tanh(x)
        x = nn.Dense(self.nr_atoms, kernel_init=orthogonal(1), bias_init=constant(0.0))(x)
        return x
...
class VectorCritic(nn.Module):
    ...
    def __call__(self, obs: jnp.ndarray):
        ...
        q_values = vmap_critic(nr_atoms=self.nr_atoms_per_net, nr_hidden_units=self.nr_hidden_units)(obs)
        return q_values