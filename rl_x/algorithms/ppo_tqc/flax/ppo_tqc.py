...
from rl_x.algorithms.ppo_tqc.flax.policy import get_policy
from rl_x.algorithms.ppo_tqc.flax.critic import get_critic
from rl_x.algorithms.ppo_tqc.flax.agent_params import AgentParams
from rl_x.algorithms.ppo_tqc.flax.storage import Storage
...
class PPO_TQC:
    ...
    def train(self):
        ...
        def get_action_and_value(params: flax.core.FrozenDict, state: np.ndarray, storage: Storage, step: int, key: jax.random.PRNGKey):
            ...
            value = self.critic.apply(params.critic_target_params, state)
            value = jnp.transpose(value, (1, 0, 2)).reshape(self.nr_envs, self.nr_total_atoms)
        ...
        def get_action_and_value2(params: flax.core.FrozenDict, state: np.ndarray, action: np.ndarray):
            ...
            value = jnp.transpose(value, (1, 0, 2)).reshape(self.minibatch_size, self.nr_total_atoms)
        ...
        def calculate_gae_advantages(params: flax.core.FrozenDict, next_states: jnp.array, rewards: np.ndarray, dones: np.ndarray, storage: Storage):
            rewards = jnp.expand_dims(rewards, axis=2)
            dones = jnp.expand_dims(dones, axis=2)
            next_values = jax.lax.stop_gradient(self.critic.apply(params.critic_target_params, next_states))
            next_values = jnp.transpose(next_values, (1, 2, 0, 3)).reshape(self.nr_steps, self.nr_envs, self.nr_total_atoms)
            ...
            advantages_mean = jnp.mean(advantages, axis=2)
            storage = storage.replace(
        ...
        def update(train_state: TrainState, storage: Storage, key: jax.random.PRNGKey):
            batch_states = storage.states.reshape((self.batch_size,) + self.os_shape)
            batch_actions = storage.actions.reshape((self.batch_size,) + self.as_shape)
            batch_advantages = storage.advantages.reshape(self.batch_size)
            batch_returns = storage.returns.reshape(self.batch_size, self.nr_total_atoms)
            batch_log_probs = storage.log_probs.reshape(self.batch_size)

            def ppo_loss(params, minibatch_states, minibatch_actions, minibatch_log_probs, minibatch_advantages, minibatch_returns):
                new_log_prob, entropy, v_atoms = get_action_and_value2(params, minibatch_states, minibatch_actions)
                ...
                y_atoms = jnp.sort(minibatch_returns)
                y_atoms = y_atoms[:, :self.nr_target_atoms]
                y_atoms = jnp.expand_dims(y_atoms, axis=1)  # (batch_size, 1, nr_target_atoms)

                v_atoms = jnp.expand_dims(v_atoms, axis=2)  # (batch_size, nr_total_atoms, 1)
                ...
                v_loss = jnp.abs(cumulative_prob - (delta_i_j < 0).astype(jnp.float32)) * huber_loss / self.huber_kappa
                v_loss = v_loss.mean()
            ...
            for start in range(0, self.batch_size, self.minibatch_size):
                ...
                train_state = train_state.replace(params=params.replace(critic_target_params=optax.incremental_update(params.critic_params, params.critic_target_params, self.tau)))
        ...
        storage = Storage(
            ...
            values=jnp.zeros((self.nr_steps, self.nr_envs, self.nr_total_atoms)),
            advantages=jnp.zeros((self.nr_steps, self.nr_envs)),
            returns=jnp.zeros((self.nr_steps, self.nr_envs, self.nr_total_atoms)),
        )
    ...
    def load(config, env, run_path, writer):
        ...
        model = PPO_TQC(config, env, run_path, writer)
