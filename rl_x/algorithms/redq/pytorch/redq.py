...
from rl_x.algorithms.redq.pytorch.policy import get_policy
from rl_x.algorithms.redq.pytorch.critic import get_critic
from rl_x.algorithms.redq.pytorch.entropy_coefficient import get_entropy_coefficient
from rl_x.algorithms.redq.pytorch.replay_buffer import ReplayBuffer
...
class REDQ:
    ...
    def train(self):
        ...
        def critic_loss_fn(states, next_states, actions, rewards, dones):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                with torch.no_grad():
                    ...
                    target_indices = torch.randperm(self.ensemble_size, device=self.device)[:self.in_target_minimization_size]
                    next_q_target = self.critic.q_target(next_states, next_actions)[target_indices].min(dim=0).values
        ...
        while global_step < self.total_timesteps:
            ...
            if should_optimize:
                ...
                policy_loss, entropy_loss, min_q, entropy, alpha, policy_grad_norm, entropy_grad_norm = policy_and_entropy_loss_fn(batch_states[0])
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = REDQ(config, train_env, eval_env, run_path, writer)
