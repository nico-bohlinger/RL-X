...
class VectorQNetwork(nn.Module):
    def __init__(self, config, env, device, critic_observation_indices):
        super().__init__()
        self.critics = nn.ModuleList([QNetwork(env, config.algorithm.nr_hidden_units, device, critic_observation_indices) for _ in range(config.algorithm.ensemble_size)])
