...
from rl_x.algorithms.ddpg.pytorch.policy import get_policy
from rl_x.algorithms.ddpg.pytorch.critic import get_critic
from rl_x.algorithms.ddpg.pytorch.replay_buffer import ReplayBuffer
...
class DDPG:
    ...
    def train(self):
        ...
        def critic_loss_fn(states, next_states, actions, rewards, terminations):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                with torch.no_grad():
                    ...
                    y = rewards.reshape(-1, 1) + self.gamma * (1 - terminations.reshape(-1, 1)) * next_q_target
        ...
        def policy_loss_fn(states):
            ...
            return policy_loss, q.mean(), policy_grad_norm
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = DDPG(config, train_env, eval_env, run_path, writer)
