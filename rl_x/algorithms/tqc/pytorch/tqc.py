...
from rl_x.algorithms.tqc.pytorch.policy import get_policy
from rl_x.algorithms.tqc.pytorch.critic import get_critic
from rl_x.algorithms.tqc.pytorch.entropy_coefficient import get_entropy_coefficient
from rl_x.algorithms.tqc.pytorch.replay_buffer import ReplayBuffer
    ...
    def train(self):
        ...
        def critic_loss_fn(states, next_states, actions, rewards, dones):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                with torch.no_grad():
                    ...
                    next_q_target_atoms = self.critic.q_target(next_states, next_actions).permute(1, 0, 2).reshape(next_states.shape[0], self.nr_total_atoms)
                    next_q_target_atoms = next_q_target_atoms.sort(dim=-1).values[:, :self.nr_target_atoms]
                    alpha = self.entropy_coefficient().detach()
                    y = rewards.reshape(-1, 1) + self.gamma * (1 - dones.reshape(-1, 1)) * (next_q_target_atoms - alpha * next_log_probs)

                q_atoms = self.critic.q(states, actions).permute(1, 0, 2).reshape(states.shape[0], self.nr_total_atoms)
                delta = y.unsqueeze(1) - q_atoms.unsqueeze(2)
                abs_delta = delta.abs()
                huber_loss = torch.where(abs_delta <= self.huber_kappa, 0.5 * delta ** 2, self.huber_kappa * (abs_delta - 0.5 * self.huber_kappa))
                cumulative_prob = (torch.arange(self.nr_total_atoms, device=self.device, dtype=torch.float32) + 0.5) / self.nr_total_atoms
                q_loss = (torch.abs(cumulative_prob.reshape(1, -1, 1) - (delta < 0).float()) * huber_loss / self.huber_kappa).mean()
        ...
        while global_step < self.total_timesteps:
            ...
            if should_optimize:
                ...
                policy_loss, entropy_loss, mean_q, entropy, alpha, policy_grad_norm, entropy_grad_norm = policy_and_entropy_loss_fn(batch_states)
