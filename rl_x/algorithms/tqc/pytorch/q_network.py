...
class QNetwork(nn.Module):
    def __init__(self, env, nr_atoms, nr_hidden_units, device, critic_observation_indices):
        super().__init__()
        ...
        self.critic = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(nr_hidden_units, nr_atoms),
        )
...
class VectorQNetwork(nn.Module):
    def __init__(self, config, env, device, critic_observation_indices):
        super().__init__()
        self.critics = nn.ModuleList([QNetwork(env, config.algorithm.nr_atoms_per_net, config.algorithm.nr_hidden_units, device, critic_observation_indices) for _ in range(config.algorithm.ensemble_size)])
