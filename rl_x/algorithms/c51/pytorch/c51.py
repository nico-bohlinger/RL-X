...
from rl_x.algorithms.c51.pytorch.critic import get_critic
from rl_x.algorithms.c51.pytorch.replay_buffer import ReplayBuffer
        ...
        def update(states, next_states, actions, rewards, terminations):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                with torch.no_grad():
                    ...
                    target_z = torch.clamp(rewards.unsqueeze(1) + self.gamma * (1 - terminations.unsqueeze(1)) * self.q_support, self.v_min, self.v_max)
                    b = (target_z - self.v_min) / delta_z
                    lower = b.floor().long()
                    upper = b.ceil().long()
                    equal = upper == lower
                    lower_mask = equal & (lower > 0)
                    upper_mask = equal & (lower == 0)
                    ...
                    next_dist = self.critic_target(next_states).softmax(dim=-1)
                    next_q_values = (next_dist * self.q_support).sum(dim=-1)
                    next_action = next_q_values.argmax(dim=-1)
                    next_action_dist = next_dist[torch.arange(next_dist.shape[0], device=self.device), next_action]
                    projected_dist = torch.zeros_like(next_action_dist)
                    projected_dist.scatter_add_(1, lower, next_action_dist * lower_weight)
                    projected_dist.scatter_add_(1, upper, next_action_dist * upper_weight)

                current_logits = self.critic(states)[torch.arange(states.shape[0], device=self.device), actions.reshape(-1)]
                q_loss = -(projected_dist * current_logits.log_softmax(dim=-1)).sum(dim=-1).mean()
                q = (current_logits.softmax(dim=-1) * self.q_support).sum(dim=-1)
        ...
        while global_step < self.total_timesteps:
            ...
            if global_step < self.learning_starts:
                ...
                with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                    action_dist = self.critic(torch.tensor(state, dtype=torch.float32, device=self.device)).softmax(dim=-1)
                    greedy_action = (action_dist * self.q_support).sum(dim=-1).argmax(dim=-1).cpu().numpy()
            ...
            if should_evaluate:
                ...
                while True:
                    torch.compiler.cudagraph_mark_step_begin()
                    with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        eval_action_dist = self.critic(torch.tensor(eval_state, dtype=torch.float32, device=self.device)).softmax(dim=-1)
                        eval_action = (eval_action_dist * self.q_support).sum(dim=-1).argmax(dim=-1).cpu().numpy()
    ...
    def test(self, episodes):
        ...
        for i in range(episodes):
            ...
            while not done:
                ...
                with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                    action_dist = self.critic(torch.tensor(state, dtype=torch.float32, device=self.device)).softmax(dim=-1)
                    action = (action_dist * self.q_support).sum(dim=-1).argmax(dim=-1).cpu().numpy()
