...
class ReplayBuffer():
    ...
    def sample(self, nr_samples):
        ...
        states = torch.tensor(self.states[idx1, idx2], dtype=torch.float32, device=self.device).reshape((nr_samples,) + self.os_shape)
        next_states = torch.tensor(self.next_states[idx1, idx2], dtype=torch.float32, device=self.device).reshape((nr_samples,) + self.os_shape)
        actions = torch.tensor(self.actions[idx1, idx2], dtype=torch.int64, device=self.device).reshape((nr_samples,) + self.as_shape)
        rewards = torch.tensor(self.rewards[idx1, idx2], dtype=torch.float32, device=self.device).reshape((nr_samples,))
        terminations = torch.tensor(self.terminations[idx1, idx2], dtype=torch.float32, device=self.device).reshape((nr_samples,))
