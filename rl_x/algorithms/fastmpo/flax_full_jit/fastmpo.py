...
from rl_x.algorithms.fastmpo.flax_full_jit import critic_support
...
class FastMPO:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.adaptive_critic_support = config.algorithm.adaptive_critic_support
        self.critic_support_statistics_rate = config.algorithm.critic_support_statistics_rate
        self.critic_support_update_rate = config.algorithm.critic_support_update_rate
        self.critic_support_stddev_multiplier = config.algorithm.critic_support_stddev_multiplier
        self.critic_support_min_half_range = config.algorithm.critic_support_min_half_range
        self.critic_support_max_half_range = config.algorithm.critic_support_max_half_range
        ...
        if self.adaptive_critic_support and self.critic_support_statistics_rate <= 0.0:
            raise ValueError("Critic support statistics rate must be positive.")

        if self.adaptive_critic_support and self.critic_support_update_rate <= 0.0:
            raise ValueError("Critic support update rate must be positive.")

        if self.adaptive_critic_support and self.critic_support_stddev_multiplier <= 0.0:
            raise ValueError("Critic support standard deviation multiplier must be positive.")

        if self.adaptive_critic_support and (
            self.critic_support_min_half_range <= 0.0
            or self.critic_support_max_half_range < self.critic_support_min_half_range
        ):
            raise ValueError("Critic support half-range bounds are invalid.")
        ...
        self.critic_support_state = critic_support.init_critic_support_state(
            self.v_min, self.v_max, self.critic_support_stddev_multiplier
        )
    ...
    def train(self):
        def jitable_train_function(key, parallel_seed_id):
            ...
            critic_support_state = self.critic_support_state
            ...
            def fill_replay_buffer(carry, _):
                policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key = carry
                ...
                return (policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key), None
            
            key, subkey = jax.random.split(key)
            fill_replay_buffer_carry, _ = jax.lax.scan(fill_replay_buffer, (policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, subkey), jnp.arange(self.learning_starts))
            policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key = fill_replay_buffer_carry


            # Training
            def eval_save_iteration(eval_save_iteration_carry, eval_save_iteration_step):
                policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key = eval_save_iteration_carry

                def logging_iteration(logging_iteration_carry, logging_iteration_step):
                    policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key = logging_iteration_carry

                    def learning_iteration(learning_iteration_carry, learning_iteration_step):
                        policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key = learning_iteration_carry
                        ...
                        def critic_loss_fn(policy_target_params, critic_params, critic_target_params, support_center, support_half_range, normalized_state, normalized_next_state, action, reward, done, truncated, effective_n_steps, key):
                            # Critic loss
                            ...
                            support_min = support_center - support_half_range
                            support_max = support_center + support_half_range
                            delta_z = (support_max - support_min) / (self.nr_atoms - 1)
                            q_support = support_center + support_half_range * jnp.linspace(-1.0, 1.0, self.nr_atoms)
                            bootstrap = 1.0 - (done * (1.0 - truncated))
                            discount = (self.gamma ** effective_n_steps) * bootstrap
                            raw_target_z = reward + discount * q_support
                            target_z = jnp.clip(raw_target_z, support_min, support_max)
                            
                            b = (target_z - support_min) / delta_z
                            ...
                            target_mean_per_critic = jnp.sum(mean_target_next_pmf * raw_target_z[None, :], axis=-1)
                            target_second_moment_per_critic = jnp.sum(mean_target_next_pmf * jnp.square(raw_target_z)[None, :], axis=-1)
                            support_saturation_fraction = jnp.mean(
                                jnp.sum(
                                    mean_target_next_pmf
                                    * ((raw_target_z <= support_min) | (raw_target_z >= support_max))[None, :],
                                    axis=-1,
                                )
                            )
                            ...
                            metrics = {
                                ...
                                "q/target_raw_mean": jnp.mean(target_mean_per_critic),
                                "q/target_raw_second_moment": jnp.mean(target_second_moment_per_critic),
                                "q/target_support_saturation_fraction": support_saturation_fraction,
                            }

                            return q_loss, (metrics)
                        
                        def policy_and_dual_loss_fn(policy_params, policy_target_params, critic_target_params, dual_variables_params, support_center, support_half_range, normalized_state, normalized_next_state, key1, key2):
                        ...
                        vmap_critic_loss_fn = jax.vmap(critic_loss_fn, in_axes=(None, None, None, None, None, 0, 0, 0, 0, 0, 0, 0, 0), out_axes=0)
                        ...
                        vmap_policy_and_dual_loss_fn = jax.vmap(policy_and_dual_loss_fn, in_axes=(None, None, None, None, None, None, 0, 0, 0, 0), out_axes=0)
                        ...
                        support_center = critic_support_state["center"]
                        support_half_range = critic_support_state["half_range"]
                        update_idx = 0
                        for _ in range(self.nr_policy_updates_per_step):
                            for _ in range(self.nr_critic_updates_per_policy_update):
                                ...
                                (loss, (critic_metrics)), (critic_gradients,) = grad_critic_loss_fn(
                                    ...
                                    support_center, support_half_range, normalized_states, normalized_next_states,
                                    actions, rewards, dones, truncations, effective_n_steps,
                                ...
                                if self.adaptive_critic_support:
                                    critic_support_state = critic_support.update_critic_support(
                                        critic_support_state,
                                        critic_metrics["q/target_raw_mean"],
                                        critic_metrics["q/target_raw_second_moment"],
                                        self.critic_support_statistics_rate,
                                        self.critic_support_update_rate,
                                        self.critic_support_stddev_multiplier,
                                        self.critic_support_min_half_range,
                                        self.critic_support_max_half_range,
                                        self.float_epsilon,
                                    )
                                    support_center = critic_support_state["center"]
                                    support_half_range = critic_support_state["half_range"]
                                ...
                                critic_metrics["q/support_center"] = support_center
                                critic_metrics["q/support_half_range"] = support_half_range
                                critic_metrics["q/support_min"] = support_center - support_half_range
                                critic_metrics["q/support_max"] = support_center + support_half_range
                                critic_metrics["q/support_target_running_mean"] = critic_support_state["target_running_mean"]
                                critic_metrics["q/support_target_running_std"] = jnp.sqrt(
                                    jnp.maximum(
                                        critic_support_state["target_running_second_moment"]
                                        - jnp.square(critic_support_state["target_running_mean"]),
                                        self.float_epsilon,
                                    )
                                )
                            ...
                            (loss, (policy_metrics, mean_kl_mean_for_update)), (policy_gradients, dual_variables_gradients) = grad_policy_and_dual_loss_fn(
                                ...
                                support_center, support_half_range,
                        ...
                        return (policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key), (env_state.info, metrics)
                        
                    key, subkey = jax.random.split(key)
                    learning_iteration_carry, info_and_optimization_metrics = jax.lax.scan(learning_iteration, (policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, subkey), jnp.arange(self.nr_updates_per_logging_iteration))
                    policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key = learning_iteration_carry
                ...
                key, subkey = jax.random.split(key)
                logging_iteration_carry, _ = jax.lax.scan(logging_iteration, (policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, subkey), jnp.arange(self.nr_loggings_per_eval_save_iteration))
                policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key = logging_iteration_carry
                ...
                # Saving
                if self.save_model:
                    def save_with_check(policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state):
                        self.save(policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state)
                    jax.debug.callback(save_with_check, policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state)

                
                return (policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key), None

            jax.lax.scan(eval_save_iteration, (policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state, replay_buffer, env_state, key), jnp.arange(self.nr_eval_save_iterations))
    ...
    def save(self, policy_state, critic_state, dual_variables_state, observation_normalizer_state, critic_support_state):
        checkpoint = {
            "policy": policy_state,
            "critic": critic_state,
        ...
        if self.adaptive_critic_support:
            checkpoint["critic_support"] = critic_support_state
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        if model.adaptive_critic_support:
            target["critic_support"] = model.critic_support_state
        ...
        if model.adaptive_critic_support:
            model.critic_support_state = checkpoint["critic_support"]
