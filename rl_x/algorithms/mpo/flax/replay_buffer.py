...
class ReplayBuffer():
    def __init__(self, capacity, nr_envs, trace_length, os_shape, as_shape):
        ...
        self.next_states = np.zeros((self.capacity, nr_envs, trace_length) + os_shape, dtype=np.float32)
    ...
    def sample(self, nr_samples):
        ...
        next_states = self.next_states[idx1, idx2].reshape((nr_samples, self.trace_length) + self.os_shape)
