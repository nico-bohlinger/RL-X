...
def get_q_network(config, env, device):
    ...
    if observation_space_type == ObservationSpaceType.FLAT_VALUES:
        q_network = torch.compile(QNetwork(env, config.algorithm.nr_atoms, config.algorithm.nr_hidden_units, device, critic_observation_indices).to(device), mode=compile_mode)
