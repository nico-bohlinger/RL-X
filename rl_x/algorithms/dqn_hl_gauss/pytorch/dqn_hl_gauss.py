...
from rl_x.algorithms.dqn_hl_gauss.pytorch.critic import get_critic
from rl_x.algorithms.dqn_hl_gauss.pytorch.replay_buffer import ReplayBuffer
...
class DQN_HL_Gauss:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.support = torch.linspace(self.v_min, self.v_max, self.nr_bins + 1, device=self.device)
    ...
    def train(self):
        ...
        def update(states, next_states, actions, rewards, terminations):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                with torch.no_grad():
                    next_q_target_probs = self.critic_target(next_states).softmax(dim=-1)
                    next_q_target = (next_q_target_probs * self.centers).sum(dim=-1)
                    y = rewards + self.gamma * (1 - terminations) * next_q_target.max(dim=-1).values
                    y = y.clamp(self.v_min, self.v_max)
                    cdf_evals = torch.erf((self.support.unsqueeze(0) - y.unsqueeze(1)) / (np.sqrt(2) * self.sigma))
                    target_probs = cdf_evals[:, 1:] - cdf_evals[:, :-1]
                    target_probs /= (cdf_evals[:, -1] - cdf_evals[:, 0]).unsqueeze(1)

                current_logits = self.critic(states)[torch.arange(states.shape[0], device=self.device), actions.reshape(-1)]
                q_probs = current_logits.softmax(dim=-1)
                q_loss = -(target_probs * torch.log(q_probs + 1e-8)).sum(dim=-1).mean()
                q = (q_probs * self.centers).sum(dim=-1)
        ...
        while global_step < self.total_timesteps:
            ...
            if global_step < self.learning_starts:
                ...
                with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                    ...
                    greedy_action = (action_dist * self.centers).sum(dim=-1).argmax(dim=-1).cpu().numpy()
            ...
            if should_evaluate:
                ...
                while True:
                    ...
                    with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        ...
                        eval_action = (eval_action_dist * self.centers).sum(dim=-1).argmax(dim=-1).cpu().numpy()
    ...
    def test(self, episodes):
        ...
        for i in range(episodes):
            ...
            while not done:
                ...
                with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                    action_dist = self.critic(torch.tensor(state, dtype=torch.float32, device=self.device)).softmax(dim=-1)
                    action = (action_dist * self.centers).sum(dim=-1).argmax(dim=-1).cpu().numpy()
