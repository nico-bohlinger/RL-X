...
from rl_x.algorithms.ppo_history_window.pytorch.policy import get_policy
from rl_x.algorithms.ppo_history_window.pytorch.critic import get_critic
from rl_x.algorithms.ppo_history_window.pytorch.batch import Batch
...
class PPO_History_Window:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.window_length = config.algorithm.window_length
    ...
    def train(self):
        ...
        batch = Batch(
            ...
            init_policy_window=torch.zeros((self.nr_envs, self.window_length, self.os_shape[0]), dtype=torch.float32, device=self.device),
        )
        ...
        while global_step < self.total_timesteps:
            ...
            with torch.inference_mode():
                ...
                batch.init_policy_window.copy_(rolling_obs_window)
                for step in range(self.nr_steps):
                    torch.compiler.cudagraph_mark_step_begin()
                    with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        action, processed_action, log_prob, next_rolling_obs_window = self.policy.get_action_logprob(state, rolling_obs_window)
                    ...
                    state = next_state
                    rolling_obs_window = next_rolling_obs_window * (1 - batch.dones[step, :, None, None])
            ...
            for epoch in range(self.nr_epochs):
                ...
                for start in range(0, self.nr_envs, self.nr_minibatch_envs):
                    ...
                    pg_loss, entropy_loss, approx_kl_div, clip_fraction, policy_grad_norm = policy_loss_fn(batch.states[:, minibatch_env_indices], batch.actions[:, minibatch_env_indices], batch.log_probs[:, minibatch_env_indices], batch.advantages[:, minibatch_env_indices], batch.dones[:, minibatch_env_indices], batch.init_policy_window[minibatch_env_indices])
            ...
            if global_step % self.evaluation_frequency == 0 and self.evaluation_frequency != -1:
                with torch.inference_mode():
                    ...
                    eval_rolling_obs_window = self.policy.initialize_window(eval_state.shape[0], self.os_shape[0])
                    eval_nr_episodes = 0
                    evaluation_metrics = {"eval/episode_return": [], "eval/episode_length": []}
                    while True:
                        torch.compiler.cudagraph_mark_step_begin()
                        with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                            eval_processed_action, next_eval_rolling_obs_window = self.policy.get_deterministic_action(eval_state, eval_rolling_obs_window)
                        ...
                        eval_rolling_obs_window = next_eval_rolling_obs_window * (1 - torch.tensor(eval_done, dtype=torch.float32, device=self.device)[:, None, None])
    ...
    def test(self, episodes):
        with torch.inference_mode():
            ...
            for episode in range(episodes):
                ...
                state = torch.tensor(state, dtype=torch.float32, device=self.device)
                rolling_obs_window = self.policy.initialize_window(state.shape[0], self.os_shape[0])
                while not done:
                    torch.compiler.cudagraph_mark_step_begin()
                    with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        processed_action, next_rolling_obs_window = self.policy.get_deterministic_action(state, rolling_obs_window)
                    state, reward, terminated, truncated, info = self.eval_env.step(processed_action.float().cpu().numpy())
                    done = terminated | truncated
                    state = torch.tensor(state, dtype=torch.float32, device=self.device)
                    rolling_obs_window = next_rolling_obs_window * (1 - torch.tensor(done, dtype=torch.float32, device=self.device)[:, None, None])
