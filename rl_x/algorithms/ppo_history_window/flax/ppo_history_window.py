...
class PPO_History_Window:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        dummy_rolling_obs_window = self.policy.initialize_window(1, self.os_shape[0])

        self.policy_state = TrainState.create(
            apply_fn=self.policy.apply,
            params=self.policy.init(policy_key, state, dummy_rolling_obs_window, method=self.policy.apply_one_step),
            tx=optax.chain(
    ...
    def train(self):
        ...
        def update(policy_state: TrainState, critic_state: TrainState, init_policy_window: np.ndarray,
            ...
            def minibatch_update(carry, minibatch_env_indices):
                ...
                (loss, (metrics)), (policy_gradients, critic_gradients) = grad_loss_fn(
                    ...
                    init_policy_window[minibatch_env_indices]
                )
        ...
        batch = Batch(
            ...
            init_policy_window=np.zeros((self.nr_envs, self.window_length, self.os_shape[0])),
        )
        ...
        while global_step < self.total_timesteps:
            ...
            batch.init_policy_window = np.array(rolling_obs_window)
            for step in range(self.nr_steps):
                processed_action, action, value, log_prob, next_rolling_obs_window, self.key = get_action_and_value(self.policy_state, self.critic_state, state, rolling_obs_window, self.key)
                next_state, reward, terminated, truncated, info = self.train_env.step(jax.device_get(processed_action))
                done = terminated | truncated
                next_rolling_obs_window = next_rolling_obs_window * (1.0 - jnp.array(done)[:, None, None])
            ...
            self.policy_state, self.critic_state, optimization_metrics, self.key = update(
                self.policy_state, self.critic_state, batch.init_policy_window,
            ...
            if global_step % self.evaluation_frequency == 0 and self.evaluation_frequency != -1:
                ...
                while True:
                    eval_processed_action, next_eval_rolling_obs_window = get_deterministic_action(self.policy_state, eval_state, eval_rolling_obs_window)
                    ...
                    eval_rolling_obs_window = next_eval_rolling_obs_window * (1.0 - np.array(eval_done)[:, None, None])
