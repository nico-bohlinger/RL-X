...
def get_critic(config, env):
    ...
    if observation_space_type == ObservationSpaceType.FLAT_VALUES:
        return VectorCritic(config.algorithm.nr_atoms_per_net, config.algorithm.nr_hidden_units, config.algorithm.nr_heads_per_net, config.algorithm.ensemble_size)
    else:
...
class Critic(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, a: jnp.ndarray):
        ...
        x = nn.Dense(self.nr_hidden_units)(x)
        x = nn.relu(x)
        x = nn.Dense(self.nr_atoms * self.nr_heads_per_net)(x)
        return x
...
class VectorCritic(nn.Module):
    ...
    def __call__(self, obs: jnp.ndarray, action: jnp.ndarray):
        ...
        q_values = vmap_critic(nr_atoms=self.nr_atoms_per_net, nr_hidden_units=self.nr_hidden_units, nr_heads_per_net=self.nr_heads_per_net)(obs, action)
        return q_values
