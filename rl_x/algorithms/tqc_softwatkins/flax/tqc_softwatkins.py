...
from rl_x.algorithms.tqc_softwatkins.flax.policy import get_policy
from rl_x.algorithms.tqc_softwatkins.flax.critic import get_critic
from rl_x.algorithms.tqc_softwatkins.flax.entropy_coefficient import EntropyCoefficient, ConstantEntropyCoefficient
from rl_x.algorithms.tqc_softwatkins.flax.replay_buffer import ReplayBuffer
from rl_x.algorithms.tqc_softwatkins.flax.rl_train_state import RLTrainState

rlx_logger = logging.getLogger("rl_x")


class TQC_SoftWatkinsQLambda():
    ...
    def train(self):
        ...
        def update(
            ...
            for i in range(self.q_update_steps):
                ...
                traces_ = self.q_lambda * jnp.where(current_q_target_atoms[:, 1:, :] >= next_q_target_atoms[:, :-1, :] - self.soft_watkins_kappa * jnp.abs(next_q_target_atoms[:, :-1, :]), 1.0, 0.0)  # (batch_size, trace_length - 1, nr_target_atoms)
                traces_ *= (1 - dones[i][:, :-1].reshape(self.batch_size, self.trace_length-1, 1))  # set trace to 0 if previous state was terminal
                traces = jnp.concatenate([jnp.ones((self.batch_size, 1, self.nr_target_atoms)), traces_], axis=1)  # (batch_size, trace_length, nr_target_atoms)
            ...
            return jnp.array(q_losses), jnp.array(avg_trace_coefficients), jnp.array(policy_losses), jnp.array(entropies), jnp.array(alphas), jnp.array(entropy_losses), policy_state, vector_critic_state, entropy_coefficient_state, key
        ...
        while global_step < self.total_timesteps:
            ...
            if should_optimize:
                ...
                avg_trace_coefficient_buffer.append(avg_trace_coefficients)
    ...
    def load(config, env, run_path, writer):
        ...
        model = TQC_SoftWatkinsQLambda(config, env, run_path, writer)
