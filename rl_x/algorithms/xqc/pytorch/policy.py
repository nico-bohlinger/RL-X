...
class Policy(nn.Module):
    def __init__(self, config, env, device, policy_observation_indices):
        ...
        for _ in range(config.algorithm.policy_nr_blocks):
            self.blocks.append(XQCBlock(input_dim, config.algorithm.policy_hidden_dim, config.algorithm.skip_connections))
            input_dim = config.algorithm.policy_hidden_dim
        self.mean = nn.Linear(input_dim, action_dim)
        self.log_std = nn.Linear(input_dim, action_dim)
        nn.init.orthogonal_(self.mean.weight, np.sqrt(2))
        nn.init.zeros_(self.mean.bias)
        nn.init.orthogonal_(self.log_std.weight, 1.0)
        nn.init.zeros_(self.log_std.bias)


    def forward(self, x, train, update_stats=True):
    ...
    def get_action(self, x, train=False, update_stats=True):
        mean, log_std = self(x, train, update_stats)
