...
from rl_x.algorithms.xqc.pytorch.policy import get_policy
from rl_x.algorithms.xqc.pytorch.critic import get_critic
from rl_x.algorithms.xqc.pytorch.entropy_coefficient import get_entropy_coefficient
from rl_x.algorithms.xqc.pytorch.layers import weight_norm
from rl_x.algorithms.xqc.pytorch.replay_buffer import ReplayBuffer
...
class XQC:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        if self.use_weight_norm:
            weight_norm(self.policy, self.normalize_last_layer)
            weight_norm(self.critic, self.normalize_last_layer)
            self.target_critic.load_state_dict(self.critic.state_dict())

        policy_decay_parameters = []
        policy_parameters = []
        for name, parameter in self.policy.named_parameters():
            decay = config.algorithm.decay_bn and "batch_norm" in name
            decay = decay or (not self.normalize_last_layer and any(marker in name for marker in ["mean.weight", "mean.bias", "log_std.weight", "log_std.bias"]))
            (policy_decay_parameters if decay else policy_parameters).append(parameter)
        critic_decay_parameters = []
        critic_parameters = []
        for name, parameter in self.critic.named_parameters():
            decay = config.algorithm.decay_bn and "batch_norm" in name
            decay = decay or (not self.normalize_last_layer and any(marker in name for marker in ["value.weight", "value.bias"]))
            (critic_decay_parameters if decay else critic_parameters).append(parameter)

        fused = self.device.type == "cuda"
        self.policy_optimizer = optim.AdamW([
            {"params": policy_parameters, "weight_decay": 0.0},
            {"params": policy_decay_parameters, "weight_decay": self.weight_decay},
        ], lr=self.policy_learning_rate, fused=fused)
        self.critic_optimizer = optim.AdamW([
            {"params": critic_parameters, "weight_decay": 0.0},
            {"params": critic_decay_parameters, "weight_decay": self.weight_decay},
        ], lr=self.critic_learning_rate, fused=fused)
        self.entropy_optimizer = optim.Adam([self.entropy_coefficient.log_alpha], lr=self.entropy_coefficient_learning_rate, fused=fused)

        total_updates = max(1, (self.total_timesteps // self.nr_envs) * self.updates_per_step)
        self.policy_scheduler = optim.lr_scheduler.LinearLR(self.policy_optimizer, start_factor=1.0, end_factor=self.learning_rate_end / self.policy_learning_rate, total_iters=total_updates)
        self.critic_scheduler = optim.lr_scheduler.LinearLR(self.critic_optimizer, start_factor=1.0, end_factor=self.learning_rate_end / self.critic_learning_rate, total_iters=total_updates)
        self.entropy_scheduler = optim.lr_scheduler.LinearLR(self.entropy_optimizer, start_factor=1.0, end_factor=self.learning_rate_end / self.entropy_coefficient_learning_rate, total_iters=total_updates)
    ...
    def train(self):
        ...
        def critic_loss_fn(states, next_states, actions, rewards, terminations):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                with torch.no_grad():
                    ...
                    target_values, target_log_probabilities = self.target_critic(all_observations, all_actions, True, False)
                    next_values = target_values[:, states.shape[0]:]
                    ...
                    bin_values = torch.linspace(self.v_min, self.v_max, self.nr_atoms, dtype=torch.float32, device=self.device)
                    discount = self.gamma * (1.0 - terminations)
                    target_bin_values = rewards[:, None] + discount[:, None] * (bin_values[None, :] - self.entropy_coefficient().detach() * next_log_probs[:, None])
                    target_bin_values = torch.clamp(target_bin_values, self.v_min, self.v_max)
                    target_bin_indices = (target_bin_values - self.v_min) / ((self.v_max - self.v_min) / (self.nr_atoms - 1))
                    ...
                    target_probabilities = torch.zeros((states.shape[0], self.nr_atoms), dtype=torch.float32, device=self.device)
                    target_probabilities.scatter_add_(1, lower_bin_indices, next_probabilities * (1.0 - upper_bin_weights))
                    target_probabilities.scatter_add_(1, upper_bin_indices, next_probabilities * upper_bin_weights)

                _, predicted_log_probabilities = self.critic(all_observations, all_actions, True)
            ...
            return critic_loss, next_values.mean(), critic_grad_norm


        @torch.compile(mode=self.compile_mode)
        def policy_loss_fn(states):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                actions, _, log_probs = self.policy.get_action(states, True)
                q_values, _ = self.critic(states, actions, False)
        ...
        def policy_metrics_fn(states):
            with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                ...
                entropy_loss = self.entropy_coefficient.loss(entropy)
            return policy_loss, entropy_loss, q.mean(), entropy, alpha
        ...
        while global_step < self.total_timesteps:
            ...
            if global_step < self.learning_starts:
                ...
                with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                    action, processed_action, _ = self.policy.get_action(torch.tensor(state, dtype=torch.float32, device=self.device))
            ...
            if self.normalize_reward:
                reward_done = np.logical_or(terminated, truncated).astype(np.float32)
                new_G_r = self.gamma * (1.0 - reward_done) * self.reward_normalizer_state["G_r"] + reward
                sample_mean = np.mean(new_G_r)
                sample_var = np.var(new_G_r)
                sample_count = np.float32(new_G_r.shape[0])
                delta = sample_mean - self.reward_normalizer_state["rms_mean"]
                total_count = self.reward_normalizer_state["rms_count"] + sample_count
                ratio = sample_count / total_count
                new_mean = self.reward_normalizer_state["rms_mean"] + delta * ratio
                m_a = self.reward_normalizer_state["rms_var"] * self.reward_normalizer_state["rms_count"]
                m_b = sample_var * sample_count
                self.reward_normalizer_state = {
                    "G_r": new_G_r,
                    "rms_mean": new_mean,
                    "rms_var": (m_a + m_b + np.square(delta) * self.reward_normalizer_state["rms_count"] * ratio) / total_count,
            ...
            if should_optimize:
                for _ in range(self.updates_per_step):
                    ...
                    if self.normalize_reward:
                        batch_rewards = batch_rewards / (np.sqrt(self.reward_normalizer_state["rms_var"]) + 1e-8)
                    critic_loss, target_q_mean, critic_grad_norm = critic_loss_fn(batch_states, batch_next_states, batch_actions, batch_rewards, batch_terminations)
                    if self.use_weight_norm:
                        weight_norm(self.critic, self.normalize_last_layer)
                    if (nr_updates + 1) % self.target_net_update_freq == 0:
                        with torch.no_grad():
                    ...
                    do_policy = nr_updates % self.policy_delay == 0
                    if do_policy:
                        policy_loss, entropy_loss, policy_q_mean, entropy, alpha, policy_grad_norm, entropy_grad_norm = policy_loss_fn(batch_states)
                        if self.use_weight_norm:
                            weight_norm(self.policy, self.normalize_last_layer)
                        self.policy_scheduler.step()
                        self.entropy_scheduler.step()
                    else:
                        policy_loss, entropy_loss, policy_q_mean, entropy, alpha = policy_metrics_fn(batch_states)
                        policy_grad_norm = torch.tensor(0.0)
                        entropy_grad_norm = torch.tensor(0.0)
