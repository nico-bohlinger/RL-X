...
from rl_x.algorithms.simba.pytorch.policy import get_policy
from rl_x.algorithms.simba.pytorch.critic import get_critic
from rl_x.algorithms.simba.pytorch.entropy_coefficient import get_entropy_coefficient
from rl_x.algorithms.simba.pytorch.replay_buffer import ReplayBuffer
from rl_x.algorithms.simba.pytorch.normalizer import update_normalizer, update_reward_normalizer
...
class Simba:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.policy_optimizer = optim.AdamW(self.policy.parameters(), lr=self.learning_rate_init, weight_decay=self.weight_decay, fused=fused)
        self.critic_optimizer = optim.AdamW(self.critic.parameters(), lr=self.learning_rate_init, weight_decay=self.weight_decay, fused=fused)
        ...
        def critic_loss_fn(states, next_states, actions, rewards, terminations):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                with torch.no_grad():
                    ...
                    next_q = torch.min(next_q_values, dim=0).values if self.use_cdq else next_q_values
                ...
                if self.use_cdq:
                    critic_loss = ((predicted_q_values[0] - target_q) ** 2 + (predicted_q_values[1] - target_q) ** 2).mean()
                    q1_mean = predicted_q_values[0].mean()
                    q2_mean = predicted_q_values[1].mean()
                else:
                    critic_loss = ((predicted_q_values - target_q) ** 2).mean()
