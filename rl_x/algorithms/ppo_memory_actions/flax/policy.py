...
def get_policy(config, env):
    ...
    if action_space_type == ActionSpaceType.CONTINUOUS and observation_space_type == ObservationSpaceType.FLAT_VALUES:
        return (Policy(env.single_action_space.shape, config.algorithm.memory_action_dimension, config.algorithm.memory_action_mean_clip, config.algorithm.std_dev, config.algorithm.nr_hidden_units, policy_observation_indices),
...
class Policy(nn.Module):
    ...
    def __call__(self, x):
        ...
        policy_mean = nn.tanh(policy_mean)
        policy_mean = nn.Dense(np.prod(self.as_shape).item() + self.memory_action_dimension, kernel_init=orthogonal(0.01), bias_init=constant(0.0))(policy_mean)
        env_action_mean, memory_action_mean = jnp.split(policy_mean, [np.prod(self.as_shape).item()], axis=-1)
        policy_mean = jnp.concatenate([env_action_mean, jnp.clip(memory_action_mean, -self.memory_action_mean_clip, self.memory_action_mean_clip)], axis=-1)
