...
from rl_x.algorithms.ppo_memory_actions.flax.policy import get_policy
from rl_x.algorithms.ppo_memory_actions.flax.critic import get_critic
from rl_x.algorithms.ppo_memory_actions.flax.batch import Batch
...
class PPO_Memory_Actions:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        state = jnp.concatenate([jnp.array([self.train_env.single_observation_space.sample()]), jnp.zeros((1, self.memory_action_dimension), dtype=jnp.float32)], axis=-1)
    ...
    def train(self):
        ...
        def get_action_and_value(policy_state: TrainState, critic_state: TrainState, state: np.ndarray, key: jax.random.PRNGKey):
            ...
            return processed_action, action, value.reshape(-1), log_prob.sum(1), memory_action / self.memory_action_mean_clip, key
        ...
        def get_deterministic_action(policy_state: TrainState, state: np.ndarray):
            action_mean, action_logstd = self.policy.apply(policy_state.params, state)
            env_action, memory_action = jnp.split(action_mean, [self.env_action_dimension], axis=-1)
            return self.get_processed_action(env_action), memory_action / self.memory_action_mean_clip
        ...
        while global_step < self.total_timesteps:
            ...
            for step in range(self.nr_steps):
                processed_action, action, value, log_prob, next_memory_action, self.key = get_action_and_value(self.policy_state, self.critic_state, state, self.key)
                next_env_state, reward, terminated, truncated, info = self.train_env.step(jax.device_get(processed_action))
                done = terminated | truncated
                actual_next_env_state = next_env_state.copy()
                for i, single_done in enumerate(done):
                    if single_done:
                        actual_next_env_state[i] = np.array(self.train_env.get_final_observation_at_index(info, i))
                ...
                batch.next_states[step] = np.concatenate([actual_next_env_state, np.array(next_memory_action)], axis=-1)
                ...
                memory_action = np.array(next_memory_action) * (1.0 - np.array(done)[:, None])
                state = np.concatenate([next_env_state, memory_action], axis=-1)
            ...
            if global_step % self.evaluation_frequency == 0 and self.evaluation_frequency != -1:
                ...
                eval_state = np.concatenate([eval_env_state, eval_memory_action], axis=-1)
                eval_nr_episodes = 0
                evaluation_metrics = {"eval/episode_return": [], "eval/episode_length": []}
                while True:
                    eval_processed_action, next_eval_memory_action = get_deterministic_action(self.policy_state, eval_state)
                    eval_env_state, eval_reward, eval_terminated, eval_truncated, eval_info = self.eval_env.step(jax.device_get(eval_processed_action))
                    eval_done = eval_terminated | eval_truncated
                    eval_memory_action = np.array(next_eval_memory_action) * (1.0 - np.array(eval_done)[:, None])
                    eval_state = np.concatenate([eval_env_state, eval_memory_action], axis=-1)
    ...
    def test(self, episodes):
        ...
        for i in range(episodes):
            ...
            state = np.concatenate([env_state, memory_action], axis=-1)
            while not done:
                processed_action, next_memory_action = get_action(self.policy_state, state)
                env_state, reward, terminated, truncated, info = self.eval_env.step(jax.device_get(processed_action))
                done = terminated | truncated
                memory_action = np.array(next_memory_action) * (1.0 - np.array(done)[:, None])
                state = np.concatenate([env_state, memory_action], axis=-1)
