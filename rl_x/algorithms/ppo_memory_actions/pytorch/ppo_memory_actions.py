...
from rl_x.algorithms.ppo_memory_actions.pytorch.policy import get_policy
from rl_x.algorithms.ppo_memory_actions.pytorch.critic import get_critic
from rl_x.algorithms.ppo_memory_actions.pytorch.batch import Batch
...
class PPO_Memory_Actions:
    ...
    def train(self):
        ...
        env_state, _ = self.train_env.reset()
        memory_action = np.zeros((self.nr_envs, self.memory_action_dimension), dtype=np.float32)
        state = np.concatenate([env_state, memory_action], axis=-1)
        ...
        while global_step < self.total_timesteps:
            ...
            with torch.inference_mode():
                ...
                for step in range(self.nr_steps):
                    ...
                    with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        action, processed_action, log_prob, next_memory_action = self.policy.get_action_logprob(state)
                        value = self.critic.get_value(state)
                    next_env_state, reward, terminated, truncated, info = self.train_env.step(processed_action.cpu().numpy())
                    done = terminated | truncated
                    next_env_state = torch.tensor(next_env_state, dtype=torch.float32).to(self.device)
                    actual_next_env_state = next_env_state.clone()
                    for i, single_done in enumerate(done):
                        if single_done:
                            actual_next_env_state[i] = torch.tensor(np.array(self.train_env.get_final_observation_at_index(info, i), dtype=np.float32), dtype=torch.float32).to(self.device)
                            saving_return_buffer.append(self.train_env.get_final_info_value_at_index(info, "episode_return", i))
                            dones_this_rollout += 1
                    actual_next_state = torch.cat([actual_next_env_state, next_memory_action], dim=-1)
                    ...
                    memory_action = next_memory_action * (1 - torch.tensor(done, dtype=torch.float32, device=self.device).unsqueeze(-1))
                    state = torch.cat([next_env_state, memory_action], dim=-1)
            ...
            if global_step % self.evaluation_frequency == 0 and self.evaluation_frequency != -1:
                with torch.inference_mode():
                    self.set_eval_mode()
                    eval_env_state, _ = self.eval_env.reset()
                    eval_memory_action = np.zeros((eval_env_state.shape[0], self.memory_action_dimension), dtype=np.float32)
                    eval_state = torch.tensor(np.concatenate([eval_env_state, eval_memory_action], axis=-1), dtype=torch.float32).to(self.device)
                    eval_nr_episodes = 0
                    evaluation_metrics = {"eval/episode_return": [], "eval/episode_length": []}
                    while True:
                        torch.compiler.cudagraph_mark_step_begin()
                        with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                            eval_processed_action, next_eval_memory_action = self.policy.get_deterministic_action(eval_state)
                        eval_env_state, eval_reward, eval_terminated, eval_truncated, eval_info = self.eval_env.step(eval_processed_action.cpu().numpy())
                        eval_done = eval_terminated | eval_truncated
                        eval_env_state = torch.tensor(eval_env_state, dtype=torch.float32, device=self.device)
                        eval_memory_action = next_eval_memory_action * (1 - torch.tensor(eval_done, dtype=torch.float32, device=self.device).unsqueeze(-1))
                        eval_state = torch.cat([eval_env_state, eval_memory_action], dim=-1)
    ...
    def test(self, episodes):
        with torch.inference_mode():
            ...
            for i in range(episodes):
                ...
                episode_return = 0
                env_state, _ = self.eval_env.reset()
                memory_action = np.zeros((env_state.shape[0], self.memory_action_dimension), dtype=np.float32)
                state = np.concatenate([env_state, memory_action], axis=-1)
                while not done:
                    torch.compiler.cudagraph_mark_step_begin()
                    if not torch.is_tensor(state):
                        state = torch.tensor(state, dtype=torch.float32).to(self.device)
                    with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        processed_action, next_memory_action = self.policy.get_deterministic_action(state)
                    env_state, reward, terminated, truncated, info = self.eval_env.step(processed_action.cpu().numpy())
                    done = terminated | truncated
                    env_state = torch.tensor(env_state, dtype=torch.float32, device=self.device)
                    memory_action = next_memory_action * (1 - torch.tensor(done, dtype=torch.float32, device=self.device).unsqueeze(-1))
                    state = torch.cat([env_state, memory_action], dim=-1)
