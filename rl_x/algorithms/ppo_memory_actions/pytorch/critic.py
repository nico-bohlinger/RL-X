...
def get_critic(config, env, device):
    ...
    critic_observation_indices = np.concatenate([critic_observation_indices, np.arange(env.single_observation_space.shape[0], env.single_observation_space.shape[0] + config.algorithm.memory_action_dimension)])
