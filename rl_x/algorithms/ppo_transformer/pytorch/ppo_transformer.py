...
from rl_x.algorithms.ppo_transformer.pytorch.policy import get_policy
from rl_x.algorithms.ppo_transformer.pytorch.critic import get_critic
from rl_x.algorithms.ppo_transformer.pytorch.batch import Batch
...
class PPO_Transformer:
    ...
    def train(self):
        ...
        def policy_loss_fn(states, actions, log_probs, advantages, dones, init_history):
            with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                new_log_prob, entropy = self.policy.get_logprob_entropy(states, dones, init_history, actions)
                logratio = new_log_prob - log_probs
                ratio = logratio.exp()
                normalized_advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8)
                pg_loss1 = -normalized_advantages * ratio
                pg_loss2 = -normalized_advantages * torch.clamp(ratio, 1 - self.clip_range, 1 + self.clip_range)
        ...
        self.set_train_mode()
        init_policy_history = self.policy.initialize_history(self.nr_envs, self.os_shape[0])
        batch = Batch(
            ...
            init_policy_history={key: torch.zeros_like(value) for key, value in init_policy_history.items()},
        )
        ...
        while global_step < self.total_timesteps:
            ...
            with torch.inference_mode():
                ...
                for key in history:
                    batch.init_policy_history[key].copy_(history[key])
                for step in range(self.nr_steps):
                    torch.compiler.cudagraph_mark_step_begin()
                    with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        action, processed_action, log_prob, next_history = self.policy.get_action_logprob(state, history)
                    ...
                    state = next_state
                    history = {
                        "obs": torch.where(batch.dones[step, :, None, None] > 0, torch.zeros_like(next_history["obs"]), next_history["obs"]),
                        "mask": torch.where(batch.dones[step, :, None] > 0, torch.zeros_like(next_history["mask"]), next_history["mask"]),
                    }
            ...
            for epoch in range(self.nr_epochs):
                ...
                for start in range(0, self.nr_envs, self.nr_minibatch_envs):
                    ...
                    minibatch_history = {key: value[minibatch_env_indices] for key, value in batch.init_policy_history.items()}
                    pg_loss, entropy_loss, approx_kl_div, clip_fraction, policy_grad_norm = policy_loss_fn(batch.states[:, minibatch_env_indices], batch.actions[:, minibatch_env_indices], batch.log_probs[:, minibatch_env_indices], batch.advantages[:, minibatch_env_indices], batch.dones[:, minibatch_env_indices], minibatch_history)
            ...
            if global_step % self.evaluation_frequency == 0 and self.evaluation_frequency != -1:
                with torch.inference_mode():
                    ...
                    while True:
                        torch.compiler.cudagraph_mark_step_begin()
                        with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                            eval_processed_action, next_eval_history = self.policy.get_deterministic_action(eval_state, eval_history)
                        ...
                        eval_history = {
                            "obs": torch.where(eval_done[:, None, None] > 0, torch.zeros_like(next_eval_history["obs"]), next_eval_history["obs"]),
                            "mask": torch.where(eval_done[:, None] > 0, torch.zeros_like(next_eval_history["mask"]), next_eval_history["mask"]),
                        }
        ...
        with torch.inference_mode():
            ...
            for episode in range(episodes):
                ...
                while not done:
                    torch.compiler.cudagraph_mark_step_begin()
                    with autocast(device_type="cuda", dtype=torch.bfloat16, enabled=self.bf16_mixed_precision_training):
                        processed_action, next_history = self.policy.get_deterministic_action(state, history)
                    ...
                    state = torch.tensor(state, dtype=torch.float32, device=self.device)
                    history = {
                        "obs": torch.where(done[:, None, None] > 0, torch.zeros_like(next_history["obs"]), next_history["obs"]),
                        "mask": torch.where(done[:, None] > 0, torch.zeros_like(next_history["mask"]), next_history["mask"]),
                    }
