...
from rl_x.algorithms.ppo_transformer.flax.policy import get_policy
from rl_x.algorithms.ppo_transformer.flax.critic import get_critic
from rl_x.algorithms.ppo_transformer.flax.batch import Batch
...
class PPO_Transformer:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.tf_context_len = config.algorithm.tf_context_len
        ...
        if self.minibatch_size % self.nr_steps != 0:
            raise ValueError("Minibatch size must be a multiple of nr_steps for PPO_Transformer.")
        ...
        history = self.policy.initialize_history(1, self.os_shape[0])
        self.policy_state = TrainState.create(
            apply_fn=self.policy.apply,
            params=self.policy.init(policy_key, state, history, method=self.policy.apply_one_step),
    ...
    def train(self):
        @jax.jit
        def get_action_and_value(policy_state, critic_state, state, history, key):
            action_mean, action_logstd, next_history = self.policy.apply(policy_state.params, state, history, method=self.policy.apply_one_step)
            ...
            return self.get_processed_action(action), action, value.reshape(-1), log_prob.sum(-1), next_history, key


        @jax.jit
        def calculate_gae_advantages(critic_state, next_states, rewards, terminations, values):
            def compute_advantages(carry, step):
                advantage = delta[step] + self.gamma * self.gae_lambda * (1 - terminations[step]) * carry[0]
        ...
        @jax.jit
        def update(policy_state, critic_state, init_history, states, actions, advantages, returns, values, dones, log_probs, key):
            def loss_fn(policy_params, critic_params, state_sequence, action_sequence, log_prob_sequence, return_sequence, advantage_sequence, done_sequence, history):
                action_mean, action_logstd = self.policy.apply(policy_params, state_sequence, done_sequence, history, method=self.policy.forward_sequence)
                action_std = jnp.exp(action_logstd)
                new_log_prob = (-0.5 * ((action_sequence - action_mean) / action_std) ** 2 - 0.5 * jnp.log(2.0 * jnp.pi) - action_logstd).sum(-1)
                entropy = (action_logstd + 0.5 * jnp.log(2.0 * jnp.pi * jnp.e)).sum(-1)
                logratio = new_log_prob - log_prob_sequence
                ratio = jnp.exp(logratio)
                pg_loss = jnp.maximum(-advantage_sequence * ratio, -advantage_sequence * jnp.clip(ratio, 1 - self.clip_range, 1 + self.clip_range))
                critic_loss = 0.5 * (self.critic.apply(critic_params, state_sequence).squeeze(-1) - return_sequence) ** 2
                loss = pg_loss - self.entropy_coef * entropy + self.critic_coef * critic_loss
                metrics = {
                    "loss/policy_gradient_loss": pg_loss,
                    "loss/critic_loss": critic_loss,
                    "loss/entropy_loss": entropy,
                    "policy_ratio/approx_kl": (ratio - 1) - logratio,
                    "policy_ratio/clip_fraction": jnp.float32(jnp.abs(ratio - 1) > self.clip_range),
                }
                return loss, metrics

            vmap_loss_fn = jax.vmap(loss_fn, in_axes=(None, None, 1, 1, 1, 1, 1, 1, {"obs": 0, "mask": 0}), out_axes=0)
            safe_mean = lambda x: jnp.mean(x) if x is not None else x
            mean_vmapped_loss_fn = lambda *args: tree.map_structure(safe_mean, vmap_loss_fn(*args))
            ...
            def minibatch_update(carry, minibatch_env_indices):
                ...
                minibatch_history = {key: value[minibatch_env_indices] for key, value in init_history.items()}
                (loss, metrics), (policy_gradients, critic_gradients) = grad_loss_fn(policy_state.params, critic_state.params, states[:, minibatch_env_indices], actions[:, minibatch_env_indices], log_probs[:, minibatch_env_indices], returns[:, minibatch_env_indices], minibatch_advantages, dones[:, minibatch_env_indices], minibatch_history)
            ...
            mean_metrics = {metric: jnp.mean(value) for metric, value in metrics.items()}
        ...
        @jax.jit
        def get_deterministic_action(policy_state, state, history):
            action_mean, action_logstd, next_history = self.policy.apply(policy_state.params, state, history, method=self.policy.apply_one_step)
            return self.get_processed_action(action_mean), next_history
        ...
        batch = Batch(
            ...
            init_policy_history={key: np.zeros(value.shape, dtype=value.dtype) for key, value in history.items()},
        )
        ...
        while global_step < self.total_timesteps:
            ...
            batch.init_policy_history = {key: np.array(value) for key, value in history.items()}
            for step in range(self.nr_steps):
                processed_action, action, value, log_prob, next_history, self.key = get_action_and_value(self.policy_state, self.critic_state, state, history, self.key)
                ...
                state = next_state
                history = {
                    "obs": jnp.where(jnp.array(done)[:, None, None], jnp.zeros_like(next_history["obs"]), next_history["obs"]),
                    "mask": jnp.where(jnp.array(done)[:, None], jnp.zeros_like(next_history["mask"]), next_history["mask"]),
                }
            ...
            # Optimizing
            self.policy_state, self.critic_state, optimization_metrics, self.key = update(self.policy_state, self.critic_state, batch.init_policy_history, batch.states, batch.actions, batch.advantages, batch.returns, batch.values, batch.dones, batch.log_probs, self.key)
            ...
            if global_step % self.evaluation_frequency == 0 and self.evaluation_frequency != -1:
                ...
                eval_history = self.policy.initialize_history(eval_state.shape[0], self.os_shape[0])
                eval_nr_episodes = 0
                evaluation_metrics = {"eval/episode_return": [], "eval/episode_length": []}
                while True:
                    eval_processed_action, next_eval_history = get_deterministic_action(self.policy_state, eval_state, eval_history)
                    ...
                    eval_history = {
                        "obs": jnp.where(jnp.array(eval_done)[:, None, None], jnp.zeros_like(next_eval_history["obs"]), next_eval_history["obs"]),
                        "mask": jnp.where(jnp.array(eval_done)[:, None], jnp.zeros_like(next_eval_history["mask"]), next_eval_history["mask"]),
                    }
    ...
    def save(self):
        checkpoint = {"policy": self.policy_state, "critic": self.critic_state}
    ...
    def test(self, episodes):
        @jax.jit
        def get_action(policy_state, state, history):
            action_mean, action_logstd, next_history = self.policy.apply(policy_state.params, state, history, method=self.policy.apply_one_step)
            return self.get_processed_action(action_mean), next_history
        ...
        for episode in range(episodes):
            ...
            history = self.policy.initialize_history(state.shape[0], self.os_shape[0])
            while not done:
                processed_action, next_history = get_action(self.policy_state, state, history)
