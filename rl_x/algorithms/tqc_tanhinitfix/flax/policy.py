...
from rl_x.algorithms.tqc_tanhinitfix.flax.tanh_transformed_distribution import TanhTransformedDistribution
...
class Policy(nn.Module):
    ...
    def __call__(self, x):
        ...
        mean = nn.Dense(np.prod(self.as_shape).item(), kernel_init=orthogonal(np.sqrt(2 / self.nr_hidden_units)))(x)
        log_std = nn.Dense(np.prod(self.as_shape).item(), kernel_init=orthogonal(np.sqrt(2 / self.nr_hidden_units)))(x)
