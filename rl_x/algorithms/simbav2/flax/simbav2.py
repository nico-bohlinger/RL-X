...
from rl_x.algorithms.simbav2.flax.policy import get_policy
from rl_x.algorithms.simbav2.flax.critic import get_critic
from rl_x.algorithms.simbav2.flax.entropy_coefficient import EntropyCoefficient
from rl_x.algorithms.simbav2.flax.layers import l2normalize_params
from rl_x.algorithms.simbav2.flax.replay_buffer import ReplayBuffer
from rl_x.algorithms.simbav2.flax import normalizer
...
class SimbaV2:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        policy_params = l2normalize_params(self.policy.init(policy_key, dummy_obs))
        critic_params = l2normalize_params(self.critic.init(critic_key, dummy_obs, dummy_action))
    ...
    def train(self):
        ...
        def update_step(policy_state, critic_state, target_critic_params, entropy_coefficient_state, observation_normalizer_state, reward_normalizer_state, states, next_states, actions, rewards, terms, key):
            ...
            def policy_loss_fn(policy_params):
                ...
                qs, _ = self.critic.apply(critic_state.params, states, action)
                if self.use_cdq:
            ...
            policy_state = policy_state.replace(params=l2normalize_params(policy_state.params))
            ...
            next_values, next_log_probabilities = self.critic.apply(target_critic_params, next_states, next_action)
            if self.use_cdq:
            ...
            target_bin_values = rewards[:, None] + self.gamma * (1.0 - terms[:, None]) * (
                bin_values[None, :] - (entropy_coefficient * next_log_prob)[:, None]
            )
            ...
            upper_bin_indices = jnp.clip(lower_bin_indices + 1, 0, self.nr_bins - 1)
            upper_bin_probabilities = target_bin_indices - lower_bin_indices.astype(jnp.float32)
            next_probabilities = jnp.exp(selected_next_log_probabilities)
            lower_bin_probabilities = next_probabilities * (1.0 - upper_bin_probabilities)
            upper_bin_probabilities = next_probabilities * upper_bin_probabilities
            batch_indices = jnp.broadcast_to(jnp.arange(rewards.shape[0])[:, None], lower_bin_indices.shape)
            target_probabilities = jnp.zeros((rewards.shape[0], self.nr_bins), dtype=jnp.float32)
            target_probabilities = target_probabilities.at[(batch_indices, lower_bin_indices)].add(lower_bin_probabilities)
            target_probabilities = target_probabilities.at[(batch_indices, upper_bin_indices)].add(upper_bin_probabilities)
            target_probabilities = stop_gradient(target_probabilities)

            def critic_loss_fn(critic_params):
                predicted_values, predicted_log_probabilities = self.critic.apply(critic_params, states, actions)
                if self.use_cdq:
                    cross_entropy = -jnp.sum(target_probabilities[None, :, :] * predicted_log_probabilities, axis=-1)
                    critic_loss = jnp.sum(jnp.mean(cross_entropy, axis=1))
                    q1_mean = predicted_values[0].mean()
                    q2_mean = predicted_values[1].mean()
                else:
                    critic_loss = -jnp.mean(jnp.sum(target_probabilities * predicted_log_probabilities, axis=-1))
                    q1_mean = q2_mean = predicted_values.mean()
            ...
            critic_state = critic_state.replace(params=l2normalize_params(critic_state.params))
            ...
            metrics = {
                ...
                "q/target_q_mean": jnp.mean(jnp.sum(target_probabilities * bin_values[None, :], axis=-1)),
                "q/q1_mean": q1_mean,
                "q/q2_mean": q2_mean,
